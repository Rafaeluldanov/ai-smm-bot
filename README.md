...
### Каталог платформ, иконки и demo-аналитика (v0.3.5)

Продуктовый слой: единый каталог площадок, оригинальные inline SVG-иконки и демо-аналитика
по существующим постам (подробно — [Докс/32](Докс/32_Botfleet_Каталог_платформ_и_аналитика_постов.md)).
Всё **offline**: без live API-вызовов, реальных публикаций и платежей.

- **Каталог платформ** (`platform_catalog_service.py`): 24 площадки (Россия +
  международные) с категорией, уровнем поддержки (active/beta/planned/research),
  флагами can_publish/schedule/analytics/media и requires_public_media_url. Активные:
  Telegram, VK, Instagram, Сайт, Яндекс Диск; ближайшие: YouTube, RuTube, Дзен, ОК,
  Google Drive; в планах: Facebook, TikTok, Pinterest, TenChat, VC.ru, LinkedIn, X,
  Threads, E-mail, Blog/CMS, WhatsApp Business, 2ГИС, Авито, Пикабу.
- **Оригинальные иконки** (`platform_icons.py`): inline SVG «в духе» площадок — не
  официальные логотипы, без CDN/внешних URL, `currentColor` + акценты, light/dark.
- **Дашборд** `/ui/projects/{id}/dashboard`: адаптивная сетка `.platform-grid` с
  иконками, бейджами поддержки и статусом подключения; planned-карточки «в планах», но
  кликабельные (roadmap).
- **Workspace** `/ui/projects/{id}/platforms/{platform}`: иконка + уровень поддержки,
  гайды (Telegram/VK/Instagram/Яндекс Диск), для planned — «интеграция в разработке» +
  роадмап.
- **Demo-аналитика** (`post_analytics_service.build_demo_post_analytics`): по существующим
  публикациям VK/Telegram — оценка views/reach/likes/ER/CTR/quality/engagement; источник
  метрик (`internal`/`estimated`/`demo`) всегда указан, live-вызовов нет. API
  `/analytics/projects/{id}/demo` и `/demo-summary`; UI `/ui/analytics` (summary-cards,
  календарь, demo-карточки с иконками площадок).

Без миграций (0017 — актуальный head). Live/payments off; секреты не печатаются.

### Self-service подключение платформ (v0.3.6)

Клиент подключает площадки сам в UI — **без `.env`** (подробно —
[Докс/33](Докс/33_Botfleet_Self_Service_Platform_Connections.md)). Секреты хранятся
зашифрованно в проекте, наружу — только маска; бот использует credentials проекта, а не
глобальные env-переменные. Всё offline: live-публикаций и реальных вызовов внешних API нет.

- **Хранение**: `CrmSmmResource` (миграция **0018**: `app_id`, `app_secret_encrypted/masked`,
  `status`, `last_check_*`, `resource_metadata`). Токен площадки — `api_key_encrypted`.
- **Схема формы** (`platform_connection_schema_service.py`): поля/шаги проверки/предупреждения
  по платформе (Telegram: bot token+channel; VK: access token+group+OAuth; Instagram: token+
  user id; Яндекс: публичная ссылка+root; Website: url).
- **Сервис** (`platform_connection_service.py`): upsert (секрет write-only, пустой = не менять),
  mask, delete (soft), check, `resolve_publish_credentials` (project_connection → env_fallback
  только local → missing). Токен никогда не возвращается.
- **Проверки** (`platform_connection_check_service.py`): safe read-only getMe/getChat/
  groups.getById/GET профиля/доступность ссылки; ошибка VK 27 объясняется; офлайн по
  умолчанию, онлайн-проба через `httpx.MockTransport` в тестах.
- **API** `/projects/{id}/platform-connections/*` (list/get/schema/upsert/check/delete/logs),
  все под `require_project_access`.
- **UI**: вкладка «Настройки» площадки — форма подключения (сохранить/проверить/отключить),
  маска токена, last-check; вкладка «Обзор» — «Журнал действий» (audit, без секретов).
- **Публикация**: preview показывает `credentials_source`/`token_present`; чужой проект не
  может использовать креды другого.

Миграция **0018** (platform connection fields). Live/payments off; секреты не печатаются.

### Media Proxy: публичные image_url (v0.3.7)

Foundation публичных HTTPS-ссылок на медиа — нужен площадкам с публичным `image_url`
(Instagram). Подробно — [Докс/34](Докс/34_Botfleet_Media_Proxy_Public_Image_URL.md).
Живая публикация Instagram выключена (`media_publish` не вызывается).

- **Модель** `PublicMediaLink` (миграция **0019**): `token_hash` (sha256, raw-токен НЕ
  хранится), `token_prefix`, `purpose`, `status` (active/revoked/expired), `expires_at`,
  `hit_count`, привязка к account/project/media_asset.
- **Сервис** (`media_proxy_service.py`): `create_public_link` (токен один раз,
  `secrets.token_urlsafe`), `resolve_token` (срок/отзыв/тип/размер, HEIC→JPEG,
  внутренние пути не раскрываются), `revoke_link`, `list_project_links`,
  `validate_public_base_url` (HTTPS-готовность). Base URL: `MEDIA_PROXY_PUBLIC_BASE_URL`
  → `PUBLIC_APP_URL` → `APP_BASE_URL`.
- **API** `/media-proxy/projects/{id}/…` (create link / post-links / list / status /
  revoke) под `require_project_access`; публичный `GET /media/public/{token}` без auth
  (404 на неверный/истёкший/отозванный; `Cache-Control`, `nosniff`; IP rate-limit).
- **Instagram preview**: `needs_public_image_url`, `would_prepare_public_image_url`,
  `media_proxy_enabled`, `public_media_base_url_ready`, `public_media_warning` (если base
  URL не HTTPS). Dry-run ссылок не создаёт.
- **UI**: секция «Публичные ссылки на медиа» в Instagram workspace + страница
  `/ui/projects/{id}/media-proxy` (статус, список, отзыв). **CLI**: `make media-proxy-link`,
  `make media-proxy-cleanup`. Access-log маскирует токен в пути.
...
## Worker-предложения экспериментов (v0.4.3)

Фоновый worker анализирует проект и **предлагает** эксперименты/темы, а клиент видит
«Рекомендации worker-а» (`/ui/projects/{id}/experiment-suggestions`) и решает — принять /
отклонить / скрыть / создать A/B. Предложения не публикуются live: варианты идут в очередь
ревью. Генерация предложений и accept/reject бесплатны; создание A/B из предложения — как
обычное создание A/B (идемпотентно). Дедуп по cooldown + idempotency; строгая изоляция по
проекту. Worker-генерация и авто-создание экспериментов **выключены** по умолчанию
(`EXPERIMENT_SUGGESTIONS_WORKER_ENABLED=false`, `EXPERIMENT_SUGGESTIONS_AUTO_CREATE=false`).
Подробно —
[Докс/40_Botfleet_Worker_Experiment_Suggestions.md](./Докс/40_Botfleet_Worker_Experiment_Suggestions.md).

## Автовыбор темы в worker-е (v0.4.4)

Для ближайшего слота расписания worker сам выбирает лучшую **тему / CTA / формат /
медиа-стратегию / время** на основе learning profile, метрик, feedback, A/B winners и
предложений — и сохраняет «почему бот выбрал эту тему» (`ScheduleTopicDecision`). Пост
создаётся только как draft/needs_review: **live-публикаций нет**. При низкой уверенности пост
уходит в ревью с пометкой `low_confidence`. Preview/создание решения бесплатны; автовыбор
worker-ом **выключен** по умолчанию (`AUTO_TOPIC_SELECTION_WORKER_ENABLED=false`, dry-run по
умолчанию). Клиент видит выбор на `/ui/projects/{id}/topic-decisions`. Подробно —
[Докс/41_Botfleet_Auto_Topic_Selection.md](./Докс/41_Botfleet_Auto_Topic_Selection.md).

## Автовыбор медиа в worker-е (v0.4.5)

Поверх автовыбора темы worker сам подбирает **media strategy** (`text_only` / `single_image`
/ `media_group` / `carousel_ready` / `video_later` / `no_media_available`) и **конкретные
медиа** для слота — по теме, тегам, платформе, learning profile, A/B winners, метрикам и
доступности media assets — и сохраняет «почему бот выбрал эти медиа» (`ScheduleMediaDecision`).
Пост создаётся только как draft/needs_review: **live-публикаций нет**; **публичные ссылки
автоматически не создаются**; для Instagram выставляется `needs_public_image_url=true`. При
низкой уверенности пост уходит в ревью. Preview/создание бесплатны; автовыбор worker-ом
...
## Уведомления, упоминания и нагрузка ревьюеров (v0.5.0)

Централизованные **внутренние (in-app) уведомления**: назначили задачу, упомянули (`@email` /
`@username` / `@user_id:123`), запросили правки, задача просрочена, пост ждёт ревью/одобрен/
отклонён, выбран A/B-winner, обновлён профиль обучения, worker создал рекомендацию. У каждого
пользователя есть **inbox** (`/ui/notifications`, колокольчик с бейджем непрочитанных) с
фильтрами (непрочитанные/все, тип, приоритет, проект) и действиями прочитать/скрыть.
**Упоминания** резолвятся строго в пределах аккаунта; ненайденные сохраняются как `unresolved`
и не роняют комментарий. **Нагрузка ревьюеров** (`/ui/projects/{id}/review-workload`) показывает
по каждому ответственному задачи/overdue/high-urgent/средний возраст/SLA-статус
(`ok · due_soon · overdue · critical`). Вся **внешняя доставка** (email/SMS/push/webhook)
**выключена по умолчанию** и в MVP ничего не отправляет наружу; worker выключен; уведомления не
роняют основное действие; тексты санитизируются; уведомления **бесплатны** в MVP. Подробно —
[Докс/47_Botfleet_Notifications_Mentions_Workload.md](./Докс/47_Botfleet_Notifications_Mentions_Workload.md).

## Доставка уведомлений и дайджесты (sandbox, v0.5.1)

Фундамент **внешней доставки** уведомлений (email / Telegram / webhook) и **дайджестов** —
как **sandbox/mock**. Есть delivery-задачи, журнал доставки (`pending · sent · failed · skipped ·
disabled · retry_scheduled`), retry/backoff, планировщик дайджестов (daily/weekly), настройки и
UI (`/ui/notification-delivery`, `/ui/notification-digests`). **Реальная внешняя доставка
выключена по умолчанию** и в MVP ничего не отправляет наружу: провайдеры по умолчанию `mock` и
не ходят в сеть; live-адаптеры (SMTP/Telegram/webhook) — skeleton'ы, которые отказывают, пока не
включены `NOTIFICATION_EXTERNAL_DELIVERY_ENABLED` и `*_LIVE_ENABLED` (все `false`); дайджесты и
...
— в sandbox, **без реальных Telegram API-вызовов**. Endpoint `POST /notification-telegram/webhook`
(без auth) принимает Telegram Update, проверяет secret-заголовок `X-Telegram-Bot-Api-Secret-Token`
(если включён), парсит апдейт чистым **update-парсером** (message/callback_query/edited; безопасен к
отсутствующим полям) и — если это `/start <token>` — верифицирует привязку. Ответных сообщений
наружу **не отправляет**. История апдейтов пишется в `notification_telegram_update_logs` со
статусами (received/processed/ignored/failed/verified_binding/duplicate/invalid_secret); при этом
сырой `chat_id`/`telegram_user_id` не хранятся (только sha256-hash + маска), токен в `/start`
**маскируется** (`/start abc123***`). Дубликаты по `update_id` отбрасываются. **Polling** и
**webhook management** (`setWebhook`/`deleteWebhook`/`getWebhookInfo`/`getUpdates`) — по умолчанию
**dry-run**: реальной сети нет, возвращается санитизированное описание вызова; live-методы
**отказывают**, пока не включены все флаги (external + live + bot token), а в live-пути `httpx`
импортируется лениво (в тестах — внедряемый transport, bot token не логируется). UI —
`/ui/notification-telegram` (разделы «Incoming updates / Webhook», «Проверка /start token» с формой
симуляции, «Recent incoming updates», «Webhook management (dry-run)», баннер «Реальные Telegram
API-вызовы выключены»). Всё **бесплатно** в MVP. Дефолты безопасны:
`NOTIFICATION_TELEGRAM_WEBHOOK_LIVE_ENABLED=false`, `NOTIFICATION_TELEGRAM_POLLING_LIVE_ENABLED=false`,
`NOTIFICATION_TELEGRAM_WEBHOOK_MANAGEMENT_LIVE_ENABLED=false`, `NOTIFICATION_TELEGRAM_POLLING_DRY_RUN=true`,
...
Продолжение автопилота: клиент **загружает картинки в Яндекс Диск** — Botfleet сам синхронизирует
медиатеку для автопостинга. Клиент не запускает CLI, не думает про sync/tags/fingerprints/quality —
он даёт публичную ссылку на папку и загружает фото; дальше система периодически проверяет Диск,
находит новые картинки, синхронизирует `MediaAsset`, проставляет базовые теги и прогоняет
**quality scoring → fingerprint/dedup → curation preview**, обновляя статус автопилота. Простое
клиентское состояние: «Медиа готово: N картинок / нужно добавить картинки / есть проблема».
`ProjectYandexSyncProfile` (одна панель на проект) хранит public_url (маской наружу), папку, теги и
частоту; `YandexAutoSyncRun` — историю проверок. **Безопасность — во главе**: реальная сеть и
worker **выключены по умолчанию** (`YANDEX_AUTO_SYNC_NETWORK_ENABLED=false`,
`YANDEX_AUTO_SYNC_WORKER_ENABLED=false`, `YANDEX_AUTO_SYNC_DRY_RUN=true`) — при выключенной сети
реальный sync-сервис **вообще не создаётся**, preview/run в dry-run **не пишут медиа и не ходят в
сеть**, а `run` с `dry_run=false` при выключенной сети безопасно **блокируется**. Файлы
**никогда не удаляются и не скрываются** (`AUTO_DELETE=false`, `AUTO_HIDE=false`), эндпоинта
удаления **нет**. Интеграция с автопилотом: профиль синхронизации удовлетворяет блокер
`no_yandex_disk`, а health-check автопилота использует его статус. Worker-tick встроен в
scheduler worker (выключен по умолчанию). UI — `/ui/projects/{id}/yandex-sync` («Картинки из
Яндекс Диска»: подключение, медиатека, последняя проверка, синхронизация; «Файлы не удаляются») и
упрощённая `/ui/projects/{id}/autopilot/media`. Всё **бесплатно** в MVP. Подробно —
[Докс/54_Botfleet_Yandex_Disk_Auto_Sync.md](./Докс/54_Botfleet_Yandex_Disk_Auto_Sync.md).

## Autopilot calendar assistant (v0.5.8)

Продолжение автопилота: клиент **выбирает цель и частоту** — Botfleet сам строит **календарь
автопостинга**. Клиент не думает про расписание, дни недели и времена: он говорит «хочу продажи /
3 раза в неделю» (или жмёт «Подобрать за меня»), а система предлагает готовый календарь,
распределяет дни/время, учитывает площадки, количество картинок, баланс и (если накоплены) лучшие
часы обучения. **Предварительный просмотр** показывает дни/время, число постов в месяц, сколько
нужно картинок, примерную стоимость (units/мес) и риски (мало картинок, низкий баланс, нет
площадок…). По нажатию **«Создать календарь»** сохраняется черновик `AutopilotCalendarPlan`, а
**«Применить к автопилоту»** создаёт/обновляет `CrmPublishingPlan` и активирует план — дальше
автопилот сам готовит и публикует посты. **Безопасность — во главе**: построение и применение
календаря **не публикуют** и **не включают** live-флаги (`AUTOPILOT_CALENDAR_LIVE_START_ENABLED=false`),
реальная публикация по-прежнему проходит условия безопасности автопилота; preview/create-dry-run
**ничего не пишут**, `publish_due` не вызывается, DELETE-эндпоинтов нет. Интеграция: блокер
`no_calendar` снимается при активном `CrmPublishingPlan` **или** `AutopilotCalendarPlan`, а дашборд
автопилота показывает блок `calendar_assistant`. UI — `/ui/projects/{id}/autopilot/calendar-assistant`
...
Первый безопасный **реальный live-канал** автопилота — **Telegram**. Клиент проверяет готовность,
делает предпросмотр и тестовый прогон, а когда администратор осознанно включает production-флаги —
пробует реальную публикацию. `TelegramLiveRolloutService` даёт дашборд, `preview` (без сети),
`run_once_dry` (создаёт `LivePublishAttempt`, но **никогда** не отправляет) и
`publish_once_if_allowed`. **Ключевой инвариант безопасности:** реальная отправка **невозможна** без
глобального `TELEGRAM_LIVE_PUBLISHING_ENABLED` — она срабатывает только когда **все** условия true:
`global AND project AND platform AND full_auto AND readiness_ready AND rollout allow_real_send AND
подтверждение ENABLE_TELEGRAM_LIVE AND существующие PostPublicationService gates`. По умолчанию
`TELEGRAM_LIVE_ROLLOUT_ALLOW_REAL_SEND=false` и `DRY_RUN=true` — реальная отправка выключена. Каждая
попытка (preview/blocked/skipped/attempted/published/failed) фиксируется в журнале
`LivePublishAttempt` (без токенов/сырых payload/путей) — доказательство, что **заблокированная
попытка не ходила в сеть и не списала деньги**. `ScheduleAutomationService` дополнительно журналирует
live-попытки автопилота по Telegram-слотам поверх защиты v0.5.9; дубликат блокируется по idempotency.
UI — `/ui/projects/{id}/telegram-live-rollout` («Telegram: первый live-канал автопилота», клиентский
язык, подтверждение, предупреждение про env-флаги, без кнопки прямой публикации). CLI —
`make telegram-live-rollout-dashboard|preview|run-dry|publish-once` (offline, dry-run по умолчанию).
Preview/dry-run/blocked — free; реальная публикация — существующие publication-units. Миграция
`0042_live_publish_attempts`. Тесты — только fake Telegram-клиент, реальной сети нет; `publish_due`
не вызывается. Подробно —
[Докс/57_Botfleet_Telegram_Live_Rollout.md](./Докс/57_Botfleet_Telegram_Live_Rollout.md).
