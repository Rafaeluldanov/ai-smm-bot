...
Выбранная тема превращается в черновик `Post` с текстами под Telegram, VK и Instagram, хэштегами, CTA и SEO-ключами; медиа подбирается из одобренных (`approved`/`approved_video`) или пост помечается `needs_media`. Реальный AI, автопостинг и согласование через Telegram пока не подключены.
