"""Тесты интеграции курирования медиатеки в фоновый worker (v0.4.8).

Offline; без внешнего AI/live. Проверяют дефолты (выключено), dry-run (preview без записи),
enabled (задачи), отсутствие авто-apply/delete, live-публикации и publish_due.
"""
...
def _seed(db: Session, slug: str = "wcur") -> int:
...
def test_disabled_by_default(db_session: Session) -> None:
    ...
    assert result.media_curation_enabled is False
    assert result.media_curation_tasks_created == 0
    assert media_curation_repository.list_tasks_for_project(db_session, pid) == []


def test_enabled_dry_run_previews_no_writes(db_session: Session) -> None:
    pid = _seed(db_session)
    worker = _worker(media_curation_worker_enabled=True)
    result = worker.tick(db_session, owner_id="o1", now=_NOW, dry_run=True, force=True)
    assert result.media_curation_enabled is True
    assert result.media_curation_dry_run is True
    assert result.media_curation_tasks_previewed > 0
    assert result.media_curation_tasks_created == 0
    assert media_curation_repository.list_tasks_for_project(db_session, pid) == []


def test_enabled_creates_tasks(db_session: Session) -> None:
    pid = _seed(db_session)
    worker = _worker(media_curation_worker_enabled=True, media_curation_dry_run=False)
    result = worker.tick(db_session, owner_id="o1", now=_NOW, dry_run=False, force=True)
    assert result.media_curation_enabled is True
    assert result.media_curation_tasks_created >= 1
    tasks = media_curation_repository.list_tasks_for_project(db_session, pid)
    assert len(tasks) >= 1
    # Задачи остаются proposed — никакого авто-apply.
    assert all(t.status == "proposed" for t in tasks)


def test_no_auto_apply_or_hide(db_session: Session) -> None:
    pid = _seed(db_session)
    worker = _worker(media_curation_worker_enabled=True, media_curation_dry_run=False)
    worker.tick(db_session, owner_id="o1", now=_NOW, dry_run=False, force=True)
    # Все медиа остаются selectable (ничего не скрыто автоматически); теги не применены.
    for asset in media_repo.list_media_assets_by_project(db_session, pid):
        assert asset.selection_visibility == "selectable"
...
def test_worker_no_publish_due() -> None:
    ...
    for mod in (worker_module, cur_module):
