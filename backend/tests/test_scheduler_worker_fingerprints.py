"""Тесты интеграции fingerprint/дублей в фоновый worker (v0.4.7).

Offline; без внешнего AI/сети/live. Проверяют дефолты (выключено), dry-run (preview без
записи), enabled (fingerprint созданы), отсутствие live-публикации и publish_due.
"""
...
def _seed(db: Session, slug: str = "wfp") -> int:
...
def test_disabled_by_default(db_session: Session) -> None:
    ...
    assert result.media_fingerprinting_enabled is False
    assert result.media_fingerprints_created == 0
    assert media_fingerprint_repository.list_for_project(db_session, pid) == []


def test_enabled_dry_run_previews_no_writes(db_session: Session) -> None:
    pid = _seed(db_session)
    worker = _worker(media_fingerprinting_worker_enabled=True)
    result = worker.tick(db_session, owner_id="o1", now=_NOW, dry_run=True, force=True)
    assert result.media_fingerprinting_enabled is True
    assert result.media_fingerprinting_dry_run is True
    assert result.media_fingerprints_previewed > 0
    assert result.media_fingerprints_created == 0
    assert media_fingerprint_repository.list_for_project(db_session, pid) == []


def test_enabled_creates_fingerprints(db_session: Session) -> None:
    pid = _seed(db_session)
    worker = _worker(media_fingerprinting_worker_enabled=True, media_fingerprinting_dry_run=False)
    result = worker.tick(db_session, owner_id="o1", now=_NOW, dry_run=False, force=True)
    assert result.media_fingerprinting_enabled is True
    assert result.media_fingerprints_created >= 3
    assert len(media_fingerprint_repository.list_for_project(db_session, pid)) >= 3
...
def test_worker_no_publish_due() -> None:
    from app.services import media_fingerprint_service as fp_module
    from app.services import media_similarity_service as sim_module

    for mod in (worker_module, fp_module, sim_module):
