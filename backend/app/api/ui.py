...
def ui_project_growth(project_id: int) -> HTMLResponse:
    ...
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/executive'><button class='ghost mini'>AI директор</button></a>"
...
@router.get("/projects/{project_id}/executive", response_class=HTMLResponse)
def ui_project_executive(project_id: int) -> HTMLResponse:
    """Экран «AI директор бизнеса» (Autonomous Business OS / Executive Layer v0.7.0)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/chief-of-staff'>"
        "<button class='ghost mini'>AI помощник</button></a>"
        f"<a href='/ui/projects/{project_id}/growth'><button class='ghost mini'>AI рост</button></a>"
        f"<a href='/ui/projects/{project_id}/sales-intelligence'>"
        "<button class='ghost mini'>AI продажи</button></a>"
        f"<a href='/ui/projects/{project_id}/campaigns'><button class='ghost mini'>AI кампании</button></a></div>"
        "<h2>AI директор бизнеса</h2>"
        "<p class='muted'>Верхний слой управления. Только советует и планирует: не меняет "
        "бизнес, CRM, бюджет, live и публикации без вашего подтверждения.</p>"
        "<div class='card'><div class='inline'>"
        "<button class='mini sec' onclick='exAnalyze()'>Проанализировать бизнес</button>"
        "<span id='ex-status' class='muted'></span></div>"
        "<div class='grid' style='margin-top:8px'>"
        "<div class='kv'><div>Health бизнеса</div><div id='ex-health'>—</div></div>"
        "<div class='kv'><div>Growth Score</div><div id='ex-score'>—</div></div>"
        "<div class='kv'><div>Уверенность плана</div><div id='ex-conf'>—</div></div></div>"
        "<div id='ex-summary' class='muted' style='margin-top:8px'></div></div>"
        "<div class='card'><h3>Цель бизнеса</h3><div class='inline'>"
        "<select id='ex-otype'>"
        "<option value='revenue_growth'>Рост выручки</option>"
        "<option value='lead_growth'>Рост лидов</option>"
        ...
        "<option value='expansion'>Расширение</option></select>"
        "<input id='ex-otitle' placeholder='Например: вырасти выручку x2'/>"
        "<button class='mini' onclick='exCreateObj()'>Добавить цель</button></div>"
        "<div id='ex-objs' class='muted' style='margin-top:8px'>—</div></div>"
        "<div class='grid'>"
        "<div class='card'><h3>Приоритеты</h3><div id='ex-prio' class='muted'>—</div></div>"
        "<div class='card'><h3>Возможности</h3><div id='ex-opps' class='muted'>—</div></div>"
        "<div class='card'><h3>Риски</h3><div id='ex-risks' class='muted'>—</div></div>"
        "<div class='card'><h3>Почему так</h3><div id='ex-why' class='muted'>—</div></div>"
        "</div>"
        "<div class='card'><h3>Бизнес-действия</h3><div id='ex-actions' class='muted'>—</div></div>"
    ...
    script = (
        ...
        "function lst(id,arr){const el=document.getElementById(id);el.classList.remove('muted');"
        "el.innerHTML=(arr&&arr.length)?arr.map(v=>`<div>• ${esc(''+v)}</div>`).join(''):"
        "\"<span class='muted'>пока нет данных</span>\";}"
        "function actCard(a){let b='';const s=a.status;"
        "if(s==='generated')b=`<button class='mini sec' onclick=\"exAccept(${a.id})\">Принять</button> "
        "<button class='mini ghost' onclick=\"exReject(${a.id})\">Отклонить</button>`;"
        "else if(s==='accepted')b=`<button class='mini' onclick=\"exApply(${a.id})\">Применить</button>`;"
        "return `<div class='card'><b>${esc(a.title)}</b> <span class='badge'>${esc(a.action_type)}</span> "
        "<span class='badge'>${esc(s)}</span> <span class='badge'>приоритет ${a.priority}</span>`+"
        "`<div class='muted'>${(a.reasoning||[]).map(esc).join('; ')}</div>`+"
        "`<div class='muted'>Источники: ${(a.source_modules||[]).map(esc).join(', ')}</div>`+"
        "`<div class='inline' style='margin-top:6px'>${b}</div></div>`;}"
        "async function loadObjs(){const d=await api('GET','/projects/'+PID+'/objectives');"
        "const o=d.objectives||[];document.getElementById('ex-objs').innerHTML=o.length?"
        "o.map(v=>`<div>• ${esc(v.title)} <span class='badge'>${esc(v.type)}</span> "
        "<span class='badge'>${esc(v.status)}</span></div>`).join(''):"
        "\"<span class='muted'>Целей пока нет.</span>\";}"
        "async function loadActions(){const d=await api('GET','/projects/'+PID+'/executive/actions');"
        "const a=d.actions||[];document.getElementById('ex-actions').innerHTML=a.length?"
        "a.map(actCard).join(''):\"<span class='muted'>Действий нет — запустите анализ.</span>\";}"
        "async function loadWhy(){const d=await api('GET','/projects/'+PID+'/executive/explanation');"
        "lst('ex-why',d.reasons);}"
        "async function load(){try{const p=await api('GET','/projects/'+PID+'/executive/plan');"
        "if(p.has_plan&&p.plan){const st=p.plan.current_state||{};"
        "document.getElementById('ex-health').textContent=(st.business_health!=null?st.business_health:'—');"
        "document.getElementById('ex-score').textContent=(st.growth_score!=null?st.growth_score:'—')+' / 100';"
        "document.getElementById('ex-conf').textContent=(p.plan.confidence_score||0)+' / 100';"
        "document.getElementById('ex-summary').textContent=p.plan.executive_summary||'';"
        "lst('ex-prio',p.plan.priority_actions);lst('ex-opps',p.plan.opportunities);lst('ex-risks',p.plan.risks);}"
        "await loadObjs();await loadActions();await loadWhy();}catch(x){err(eEl,x)}}"
        "async function exAnalyze(){try{document.getElementById('ex-status').textContent='Анализирую…';"
        "await api('POST','/projects/'+PID+'/executive/analyze',{});"
        "document.getElementById('ex-status').textContent='Готово.';load();}catch(x){err(eEl,x)}}"
        "async function exCreateObj(){try{const t=document.getElementById('ex-otitle').value.trim();"
        "if(!t){err(eEl,{detail:'Укажите название цели'});return;}"
        "await api('POST','/projects/'+PID+'/objectives',{type:document.getElementById('ex-otype').value,title:t});"
        "document.getElementById('ex-otitle').value='';loadObjs();}catch(x){err(eEl,x)}}"
        "async function exAccept(id){try{await api('POST','/actions/'+id+'/accept',{});loadActions();}catch(x){err(eEl,x)}}"
        "async function exReject(id){try{await api('POST','/actions/'+id+'/reject',{});loadActions();}catch(x){err(eEl,x)}}"
        "async function exApply(id){try{await api('POST','/actions/'+id+'/apply',{confirmation:'APPLY_BUSINESS_ACTION'});"
        "msg.textContent='Применено (черновик стратегии/кампании; live/CRM/бюджет не менялись).';"
        "loadActions();}catch(x){err(eEl,x)}}"
        "window.exAnalyze=exAnalyze;window.exCreateObj=exCreateObj;window.exAccept=exAccept;"
        "window.exReject=exReject;window.exApply=exApply;load();"
    )
    return _page("AI директор бизнеса", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/chief-of-staff", response_class=HTMLResponse)
def ui_project_chief_of_staff(project_id: int) -> HTMLResponse:
    """Экран «AI помощник руководителя» (AI Chief of Staff / Assistant Layer v0.7.1)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/executive'><button class='ghost mini'>AI директор</button></a>"
        f"<a href='/ui/projects/{project_id}/workflows'><button class='ghost mini'>AI процессы</button></a>"
        f"<a href='/ui/projects/{project_id}/growth'><button class='ghost mini'>AI рост</button></a></div>"
        "<h2>AI помощник руководителя</h2>"
        "<p class='muted'>Персональный ассистент: анализирует бизнес, готовит брифинг и "
        "задачи. Только советует — ничего не выполняет и не меняет сам.</p>"
        "<div class='card'><div class='inline'>"
        "<button class='mini sec' onclick='chGenerate()'>Собрать брифинг</button>"
        "<button class='mini ghost' onclick='chWeekly()'>Обзор недели</button>"
        "<span id='ch-status' class='muted'></span></div>"
        "<div id='ch-summary' class='muted' style='margin-top:8px'></div>"
        "<div class='kv' style='margin-top:6px'><div>Уверенность</div>"
        "<div id='ch-conf'>—</div></div></div>"
        "<div class='grid'>"
        "<div class='card'><h3>Сегодня — главные изменения</h3>"
        "<div id='ch-changes' class='muted'>—</div></div>"
        "<div class='card'><h3>Риски</h3><div id='ch-risks' class='muted'>—</div></div>"
        "<div class='card'><h3>Возможности</h3><div id='ch-opps' class='muted'>—</div></div>"
        "</div>"
        "<div class='card'><h3>Задачи AI</h3><div id='ch-tasks' class='muted'>—</div></div>"
        "<div class='card'><h3>Память решений</h3>"
        "<p class='muted'>Что AI знает о руководителе: стиль продаж, предпочтения, ограничения.</p>"
        "<div class='inline'>"
        "<select id='ch-dtype'>"
        ...
        "<option value='approval'>Одобрение</option></select>"
        "<input id='ch-dkey' placeholder='ключ (напр. sales_style)'/>"
        "<input id='ch-dval' placeholder='значение (напр. soft)'/>"
        "<button class='mini' onclick='chSaveDecision()'>Запомнить</button></div>"
        "<div id='ch-decisions' class='muted' style='margin-top:8px'>—</div></div>"
        "<div id='ch-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};const eEl=document.getElementById('error');"
        ...
        "el.innerHTML=(arr&&arr.length)?arr.map(v=>`<div>• ${esc(''+v)}</div>`).join(''):"
        "\"<span class='muted'>пока нет данных</span>\";}"
        "function taskCard(t){let b='';const s=t.status;"
        "if(s==='suggested')b=`<button class='mini sec' onclick=\"chAccept(${t.id})\">Принять</button> "
        "<button class='mini ghost' onclick=\"chReject(${t.id})\">Отклонить</button>`;"
        "else if(s==='accepted')b=`<button class='mini' onclick=\"chComplete(${t.id})\">Завершить</button>`;"
        "const imp=(t.expected_impact&&t.expected_impact.business)?('эффект: '+t.expected_impact.business):'';"
        "return `<div class='card'><b>${esc(t.title)}</b> <span class='badge'>${esc(t.priority)}</span> "
        "<span class='badge'>${esc(s)}</span> <span class='badge'>score ${t.priority_score}</span>`+"
        "`<div class='muted'>${(t.reasoning||[]).map(esc).join('; ')}</div>`+"
        "`<div class='muted'>${esc(imp)}</div>`+"
        "`<div class='inline' style='margin-top:6px'>${b}</div></div>`;}"
        "async function loadTasks(){const d=await api('GET','/projects/'+PID+'/tasks');"
        "const a=d.tasks||[];document.getElementById('ch-tasks').innerHTML=a.length?"
        "a.map(taskCard).join(''):\"<span class='muted'>Задач нет — соберите брифинг.</span>\";}"
        "async function loadDecisions(){const d=await api('GET','/projects/'+PID+'/ai-decisions');"
        "const a=d.decisions||[];document.getElementById('ch-decisions').innerHTML=a.length?"
        "a.map(v=>`<div>• <b>${esc(v.key)}</b>: ${esc(JSON.stringify(v.value))} "
        "<span class='badge'>${esc(v.decision_type)}</span> "
        "<button class='mini ghost' onclick=\"chDelDecision(${v.id})\">Забыть</button></div>`).join(''):"
        "\"<span class='muted'>AI пока ничего не знает о предпочтениях.</span>\";}"
        "function renderBriefing(b){if(!b){return;}"
        "document.getElementById('ch-summary').textContent=b.summary||'';"
        "document.getElementById('ch-conf').textContent=(b.confidence_score||0)+' / 100';"
        "lst('ch-changes',b.key_changes);lst('ch-risks',b.risks);lst('ch-opps',b.opportunities);}"
        "async function load(){try{const d=await api('GET','/projects/'+PID+'/briefing');"
        "if(d.has_briefing)renderBriefing(d.briefing);await loadTasks();await loadDecisions();}catch(x){err(eEl,x)}}"
        "async function chGenerate(){try{document.getElementById('ch-status').textContent='Собираю…';"
        "const d=await api('POST','/projects/'+PID+'/briefing/generate',{});"
        "renderBriefing(d.briefing);document.getElementById('ch-status').textContent='Готово.';"
        "loadTasks();}catch(x){err(eEl,x)}}"
        "async function chWeekly(){try{document.getElementById('ch-status').textContent='Считаю неделю…';"
        "const d=await api('POST','/projects/'+PID+'/briefing/weekly',{});"
        "renderBriefing(d.briefing);document.getElementById('ch-status').textContent='Готово.';"
        "loadTasks();}catch(x){err(eEl,x)}}"
        "async function chAccept(id){try{await api('POST','/tasks/'+id+'/accept',{});loadTasks();}catch(x){err(eEl,x)}}"
        "async function chReject(id){try{await api('POST','/tasks/'+id+'/reject',{});loadTasks();}catch(x){err(eEl,x)}}"
        "async function chComplete(id){try{await api('POST','/tasks/'+id+'/complete',{});"
        "msg.textContent='Задача завершена (внешних действий не выполнялось).';loadTasks();}catch(x){err(eEl,x)}}"
        "async function chSaveDecision(){try{const k=document.getElementById('ch-dkey').value.trim();"
        "if(!k){err(eEl,{detail:'Укажите ключ решения'});return;}"
        "const raw=document.getElementById('ch-dval').value.trim();"
        "await api('POST','/projects/'+PID+'/ai-decisions',{decision_type:document.getElementById('ch-dtype').value,"
        "key:k,value:{value:raw}});document.getElementById('ch-dkey').value='';"
        "document.getElementById('ch-dval').value='';loadDecisions();}catch(x){err(eEl,x)}}"
        "async function chDelDecision(id){try{await api('DELETE','/decisions/'+id);loadDecisions();}catch(x){err(eEl,x)}}"
        "window.chGenerate=chGenerate;window.chWeekly=chWeekly;window.chAccept=chAccept;"
        "window.chReject=chReject;window.chComplete=chComplete;window.chSaveDecision=chSaveDecision;"
        "window.chDelDecision=chDelDecision;load();"
    )
    return _page("AI помощник руководителя", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/workflows", response_class=HTMLResponse)
def ui_project_workflows(project_id: int) -> HTMLResponse:
    """Экран «AI процессы» (AI Workflow Manager / Business Execution Layer v0.7.2)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/operations'>"
        "<button class='ghost mini'>AI Operations</button></a>"
        f"<a href='/ui/projects/{project_id}/chief-of-staff'>"
        "<button class='ghost mini'>AI помощник</button></a>"
        f"<a href='/ui/projects/{project_id}/executive'><button class='ghost mini'>AI директор</button></a></div>"
        "<h2>AI процессы</h2>"
        "<p class='muted'>Управление процессами: цель → этапы → ответственные → прогресс → "
        "блокеры. Слой управления — ничего не выполняет и не меняет сам.</p>"
        "<div class='card'><h3>Новый процесс</h3><div class='inline'>"
        "<input id='wf-name' placeholder='название (напр. Рост продаж x2)'/>"
        "<select id='wf-type'>"
        ...
        "<input id='wf-goal' placeholder='цель'/>"
        "<button class='mini' onclick='wfCreate()'>Создать</button></div></div>"
        "<div class='card'><h3>Активные процессы</h3><div id='wf-list' class='muted'>—</div></div>"
        "<div class='card'><h3>Этапы</h3><div class='inline'>"
        "<span id='wf-current' class='muted'>Выберите процесс</span>"
        "<button class='mini sec' onclick='wfGenerate()'>Сгенерировать этапы</button></div>"
        "<div id='wf-steps' class='muted' style='margin-top:8px'>—</div></div>"
        "<div class='grid'>"
        "<div class='card'><h3>Health Score</h3><div class='kv'><div>Оценка</div>"
        "<div id='wf-health'>—</div></div><div id='wf-risks' class='muted'></div></div>"
        "<div class='card'><h3>Блокеры</h3><div id='wf-blockers' class='muted'>—</div>"
        ...
        "<option value='missing_data'>Нет данных</option>"
        "<option value='external'>Внешний</option></select>"
        "<input id='wf-btitle' placeholder='что блокирует'/>"
        "<button class='mini ghost' onclick='wfAddBlocker()'>Добавить блокер</button></div></div>"
        "</div>"
        "<div class='card'><h3>Рекомендации AI</h3><div id='wf-recs' class='muted'>—</div></div>"
        "<div id='wf-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};let CUR=null;const eEl=document.getElementById('error');"
        "const msg=document.getElementById('wf-msg');"
        "function lst(id,arr){const el=document.getElementById(id);el.classList.remove('muted');"
        "el.innerHTML=(arr&&arr.length)?arr.map(v=>`<div>• ${esc(''+v)}</div>`).join(''):"
        "\"<span class='muted'>—</span>\";}"
        "function wfCard(w){return `<div class='card'><b>${esc(w.name)}</b> "
        "<span class='badge'>${esc(w.workflow_type)}</span> <span class='badge'>${esc(w.status)}</span> "
        "<span class='badge'>прогресс ${w.progress_percent}%</span>`+"
        "`<div class='muted'>${esc(w.goal||'')}</div>`+"
        "`<div class='inline' style='margin-top:6px'>"
        "<button class='mini sec' onclick=\"wfOpen(${w.id})\">Открыть</button></div></div>`;}"
        "function stepCard(s){let b='';"
        "if(s.status!=='completed'&&s.status!=='cancelled')"
        "b=`<button class='mini' onclick=\"wfComplete(${s.id})\">Завершить</button> "
        "<button class='mini sec' onclick=\"wfProgress(${s.id})\">В работу</button>`;"
        "return `<div class='prow'>${s.order_number}. <b>${esc(s.title)}</b> "
        "<span class='badge'>${esc(s.status)}</span> <span class='badge'>${esc(s.priority)}</span> ${b}</div>`;}"
        "function blkCard(b){let x=b.status==='open'?"
        "`<button class='mini ghost' onclick=\"wfResolve(${b.id})\">Снять</button>`:'';"
        "return `<div class='prow'>🚧 <b>${esc(b.title)}</b> <span class='badge'>${esc(b.blocker_type)}</span> "
        "<span class='badge'>${esc(b.severity)}</span> <span class='badge'>${esc(b.status)}</span> ${x}</div>`;}"
        "async function loadList(){const d=await api('GET','/projects/'+PID+'/workflows');"
        "const w=d.workflows||[];document.getElementById('wf-list').innerHTML=w.length?"
        "w.map(wfCard).join(''):\"<span class='muted'>Процессов нет — создайте первый.</span>\";}"
        "async function wfOpen(id){try{CUR=id;const d=await api('GET','/workflows/'+id);"
        "document.getElementById('wf-current').textContent='Процесс: '+(d.workflow.name||('#'+id));"
        "const s=d.steps||[];document.getElementById('wf-steps').innerHTML=s.length?"
        "s.map(stepCard).join(''):\"<span class='muted'>Этапов нет — сгенерируйте.</span>\";"
        "const b=d.blockers||[];document.getElementById('wf-blockers').innerHTML=b.length?"
        "b.map(blkCard).join(''):\"<span class='muted'>Блокеров нет.</span>\";"
        "await loadHealth();}catch(x){err(eEl,x)}}"
        "async function loadHealth(){if(!CUR)return;const h=await api('GET','/workflows/'+CUR+'/health');"
        "document.getElementById('wf-health').textContent=h.health_score+' / 100';"
        "lst('wf-risks',h.risks);lst('wf-recs',h.recommendations);}"
        "async function wfCreate(){try{const n=document.getElementById('wf-name').value.trim();"
        "if(!n){err(eEl,{detail:'Укажите название процесса'});return;}"
        "await api('POST','/projects/'+PID+'/workflows',{name:n,workflow_type:document.getElementById('wf-type').value,"
        "goal:document.getElementById('wf-goal').value,status:'active'});"
        "document.getElementById('wf-name').value='';document.getElementById('wf-goal').value='';loadList();}catch(x){err(eEl,x)}}"
        "async function wfGenerate(){try{if(!CUR){err(eEl,{detail:'Откройте процесс'});return;}"
        "await api('POST','/workflows/'+CUR+'/generate-steps',{});wfOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function wfComplete(id){try{await api('POST','/steps/'+id+'/status',{status:'completed'});"
        "msg.textContent='Этап завершён (внешних действий не выполнялось).';wfOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function wfProgress(id){try{await api('POST','/steps/'+id+'/status',{status:'in_progress'});wfOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function wfAddBlocker(){try{if(!CUR){err(eEl,{detail:'Откройте процесс'});return;}"
        "const t=document.getElementById('wf-btitle').value.trim();if(!t){err(eEl,{detail:'Опишите блокер'});return;}"
        "await api('POST','/workflows/'+CUR+'/blockers',{blocker_type:document.getElementById('wf-btype').value,title:t});"
        "document.getElementById('wf-btitle').value='';wfOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function wfResolve(id){try{await api('POST','/blockers/'+id+'/resolve',{});wfOpen(CUR);}catch(x){err(eEl,x)}}"
        "window.wfCreate=wfCreate;window.wfOpen=wfOpen;window.wfGenerate=wfGenerate;window.wfComplete=wfComplete;"
        "window.wfProgress=wfProgress;window.wfAddBlocker=wfAddBlocker;window.wfResolve=wfResolve;loadList();"
    )
    return _page("AI процессы", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/operations", response_class=HTMLResponse)
def ui_project_operations(project_id: int) -> HTMLResponse:
    """Экран «AI Operations Center» (AI Operations Control Center v0.7.3)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/executive'><button class='ghost mini'>AI директор</button></a>"
        f"<a href='/ui/projects/{project_id}/decisions'><button class='ghost mini'>AI решения</button></a>"
        f"<a href='/ui/projects/{project_id}/chief-of-staff'>"
        "<button class='ghost mini'>AI помощник</button></a>"
        f"<a href='/ui/projects/{project_id}/workflows'><button class='ghost mini'>AI процессы</button></a></div>"
        "<h2>AI Operations Center</h2>"
        "<p class='muted'>Единая операционная панель: здоровье, состояние, риски, "
        "рекомендации. Только собирает сигналы и советует — ничего не выполняет.</p>"
        "<div class='card'><div class='inline'>"
        "<button class='mini sec' onclick='opAnalyze()'>Проанализировать</button>"
        "<span id='op-status' class='muted'></span></div>"
        "<div style='margin-top:8px'><span class='muted'>Health Score</span>"
        "<div id='op-health' style='font-size:2.2em;font-weight:700'>—</div>"
        "<span id='op-hstatus' class='badge'>—</span></div></div>"
        "<div class='card'><h3>Бизнес-состояние</h3><div class='grid'>"
        "<div class='kv'><div>Рост</div><div id='op-growth'>—</div></div>"
        "<div class='kv'><div>Выручка</div><div id='op-revenue'>—</div></div>"
        "<div class='kv'><div>Исполнение</div><div id='op-execution'>—</div></div>"
        "<div class='kv'><div>Процессы</div><div id='op-progress'>—</div></div></div></div>"
        "<div class='card'><h3>Риски</h3><div id='op-risks' class='muted'>—</div></div>"
        "<div class='card'><h3>AI рекомендации</h3><div id='op-recs' class='muted'>—</div></div>"
        "<div class='card'><h3>Почему такой health</h3><div id='op-why' class='muted'>—</div></div>"
    ...
    script = (
        ...
        "el.innerHTML=(arr&&arr.length)?arr.map(v=>`<div>• ${esc(''+v)}</div>`).join(''):"
        "\"<span class='muted'>—</span>\";}"
        "function riskCard(r){return `<div class='card'><b>⚠ ${esc(r.title)}</b> "
        "<span class='badge'>${esc(r.risk_type)}</span> <span class='badge'>${esc(r.severity)}</span> "
        "<span class='badge'>${esc(r.status)}</span>`+"
        "`<div class='muted'>${esc(r.description||'')}</div>`+"
        "`<div class='inline' style='margin-top:6px'>"
        "<button class='mini ghost' onclick=\"opResolve(${r.id})\">Снять</button></div></div>`;}"
        "function recCard(r){let b=r.status==='generated'?"
        "`<button class='mini sec' onclick=\"opAccept(${r.id})\">Принять</button> "
        "<button class='mini ghost' onclick=\"opReject(${r.id})\">Отклонить</button>`:'';"
        "const imp=(r.expected_impact&&r.expected_impact.health)?('эффект: '+r.expected_impact.health):'';"
        "return `<div class='card'><b>${esc(r.title)}</b> <span class='badge'>${esc(r.priority)}</span> "
        "<span class='badge'>${esc(r.status)}</span>`+"
        "`<div class='muted'>${(r.reasoning||[]).map(esc).join('; ')}</div>`+"
        "`<div class='muted'>${esc(imp)}</div>`+"
        "`<div class='inline' style='margin-top:6px'>${b}</div></div>`;}"
        "function renderSnap(s){if(!s)return;const m=s.metrics||{};"
        "document.getElementById('op-health').textContent=(s.health_score||0)+' / 100';"
        "document.getElementById('op-hstatus').textContent=s.status||'—';"
        "document.getElementById('op-growth').textContent=(m.growth!=null?m.growth:'—');"
        "document.getElementById('op-revenue').textContent=(m.revenue!=null?m.revenue:'—');"
        "document.getElementById('op-execution').textContent=(m.execution!=null?m.execution:'—');"
        "document.getElementById('op-progress').textContent=(m.workflow_progress!=null?m.workflow_progress:'—');}"
        "async function loadRisks(){const d=await api('GET','/projects/'+PID+'/operations/risks');"
        "const r=d.risks||[];document.getElementById('op-risks').innerHTML=r.length?"
        "r.map(riskCard).join(''):\"<span class='muted'>Открытых рисков нет.</span>\";}"
        "async function loadRecs(){const d=await api('GET','/projects/'+PID+'/operations/recommendations');"
        "const r=(d.recommendations||[]).filter(x=>x.status==='generated');"
        "document.getElementById('op-recs').innerHTML=r.length?r.map(recCard).join(''):"
        "\"<span class='muted'>Рекомендаций нет — запустите анализ.</span>\";}"
        "async function loadWhy(){const d=await api('GET','/projects/'+PID+'/operations/explanation');lst('op-why',d.reasons);}"
        "async function load(){try{const d=await api('GET','/projects/'+PID+'/operations');"
        "if(d.has_snapshot)renderSnap(d.snapshot);await loadRisks();await loadRecs();await loadWhy();}catch(x){err(eEl,x)}}"
        "async function opAnalyze(){try{document.getElementById('op-status').textContent='Анализирую…';"
        "const d=await api('POST','/projects/'+PID+'/operations/analyze',{});"
        "renderSnap(d.snapshot);document.getElementById('op-status').textContent='Готово.';"
        "loadRisks();loadRecs();loadWhy();}catch(x){err(eEl,x)}}"
        "async function opResolve(id){try{await api('POST','/risks/'+id+'/resolve',{});loadRisks();loadWhy();}catch(x){err(eEl,x)}}"
        "async function opAccept(id){try{await api('POST','/recommendations/'+id+'/accept',{});"
        "msg.textContent='Принято (действий не выполнялось).';loadRecs();}catch(x){err(eEl,x)}}"
        "async function opReject(id){try{await api('POST','/recommendations/'+id+'/reject',{});loadRecs();}catch(x){err(eEl,x)}}"
        "window.opAnalyze=opAnalyze;window.opResolve=opResolve;window.opAccept=opAccept;window.opReject=opReject;load();"
    )
    return _page("AI Operations Center", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/decisions", response_class=HTMLResponse)
def ui_project_decisions(project_id: int) -> HTMLResponse:
    """Экран «AI решения» (AI Decision Engine v0.7.4)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/operations'>"
        "<button class='ghost mini'>AI Operations</button></a>"
        f"<a href='/ui/projects/{project_id}/strategy-simulator'>"
        "<button class='ghost mini'>AI прогноз стратегии</button></a>"
        f"<a href='/ui/projects/{project_id}/executive'><button class='ghost mini'>AI директор</button></a></div>"
        "<h2>AI решения</h2>"
        "<p class='muted'>AI выявляет проблему, строит варианты, оценивает эффект и риск и "
        "рекомендует лучший. Только советует — решения не применяет сам.</p>"
        "<div class='card'><h3>Новое решение</h3><div class='inline'>"
        "<select id='dc-type'>"
        "<option value='growth'>Рост</option>"
        ...
        "<option value='content'>Контент</option>"
        "<option value='efficiency'>Эффективность</option>"
        "<option value='operational'>Операционное</option></select>"
        "<input id='dc-title' placeholder='проблема / название'/>"
        "<button class='mini' onclick='dcCreate()'>Создать</button></div>"
        "<div id='dc-list' class='muted' style='margin-top:8px'>—</div></div>"
        "<div class='card'><h3>Проблема</h3><div id='dc-problem' class='muted'>Выберите решение</div>"
        "<div class='inline' style='margin-top:6px'>"
        "<button class='mini sec' onclick='dcAnalyze()'>Проанализировать</button>"
        "<span id='dc-status' class='muted'></span></div></div>"
        "<div class='card'><h3>Варианты</h3><div id='dc-scenarios' class='muted'>—</div></div>"
        "<div class='card'><h3>Рекомендация AI</h3><div id='dc-rec' class='muted'>—</div>"
        "<div class='inline' style='margin-top:6px'>"
        "<button class='mini sec' onclick='dcAccept()'>Принять решение</button>"
        "<button class='mini' onclick='dcApply()'>Применить (черновик)</button></div></div>"
        "<div class='card'><h3>Почему</h3><div id='dc-why' class='muted'>—</div></div>"
        "<div id='dc-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};let CUR=null;const eEl=document.getElementById('error');"
        ...
        "el.innerHTML=(arr&&arr.length)?arr.map(v=>`<div>• ${esc(''+v)}</div>`).join(''):"
        "\"<span class='muted'>—</span>\";}"
        "function decCard(d){return `<div class='prow'>#${d.id} <b>${esc(d.title)}</b> "
        "<span class='badge'>${esc(d.decision_type)}</span> <span class='badge'>${esc(d.status)}</span> "
        "<button class='mini sec' onclick=\"dcOpen(${d.id})\">Открыть</button></div>`;}"
        "function scCard(s){let b='';if(s.status!=='rejected'&&s.status!=='selected')"
        "b=`<button class='mini sec' onclick=\"dcSelect(${s.id})\">Выбрать</button> "
        "<button class='mini ghost' onclick=\"dcReject(${s.id})\">Отклонить</button>`;"
        "const im=s.expected_impact||{};const ra=s.risk_analysis||{};"
        "return `<div class='card'><b>${esc(s.title)}</b> <span class='badge'>${esc(s.status)}</span> "
        "<span class='badge'>score ${im.score!=null?im.score:'—'}</span> "
        "<span class='badge'>эффект ${im.impact!=null?im.impact:'—'}</span> "
        "<span class='badge'>риск ${ra.risk!=null?ra.risk:'—'}</span> "
        "<span class='badge'>уверенность ${s.confidence_score}</span>`+"
        "`<div class='muted'>${esc(s.description||'')}</div>`+"
        "`<div class='inline' style='margin-top:6px'>${b}</div></div>`;}"
        "async function loadList(){const d=await api('GET','/projects/'+PID+'/ai-decisions');"
        "const a=d.decisions||[];document.getElementById('dc-list').innerHTML=a.length?"
        "a.map(decCard).join(''):\"<span class='muted'>Решений нет — создайте первое.</span>\";}"
        "async function dcOpen(id){try{CUR=id;const d=await api('GET','/ai-decisions/'+id);"
        "document.getElementById('dc-problem').textContent=d.decision.problem_statement||d.decision.title;"
        "const s=d.scenarios||[];document.getElementById('dc-scenarios').innerHTML=s.length?"
        "s.map(scCard).join(''):\"<span class='muted'>Вариантов нет — проанализируйте.</span>\";"
        "renderRec(d.decision,s);await loadWhy();}catch(x){err(eEl,x)}}"
        "function renderRec(dec,scs){const rid=dec.recommended_scenario_id;"
        "const best=(scs||[]).find(x=>x.id===rid);"
        "document.getElementById('dc-rec').innerHTML=best?"
        "`<b>${esc(best.title)}</b> — score ${(best.expected_impact||{}).score} (уверенность ${dec.confidence_score})`:"
        "\"<span class='muted'>Пока нет рекомендации.</span>\";}"
        "async function loadWhy(){if(!CUR)return;const d=await api('GET','/ai-decisions/'+CUR+'/explanation');lst('dc-why',d.reasons);}"
        "async function dcCreate(){try{const t=document.getElementById('dc-title').value.trim();"
        "if(!t){err(eEl,{detail:'Укажите название решения'});return;}"
        "await api('POST','/projects/'+PID+'/ai-decisions',{decision_type:document.getElementById('dc-type').value,"
        "title:t,problem_statement:t});document.getElementById('dc-title').value='';loadList();}catch(x){err(eEl,x)}}"
        "async function dcAnalyze(){try{if(!CUR){err(eEl,{detail:'Откройте решение'});return;}"
        "document.getElementById('dc-status').textContent='Анализирую…';"
        "await api('POST','/ai-decisions/'+CUR+'/analyze',{});"
        "document.getElementById('dc-status').textContent='Готово.';dcOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function dcSelect(id){try{await api('POST','/scenarios/'+id+'/select',{});dcOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function dcReject(id){try{await api('POST','/scenarios/'+id+'/reject',{});dcOpen(CUR);}catch(x){err(eEl,x)}}"
        "async function dcAccept(){try{if(!CUR)return;await api('POST','/ai-decisions/'+CUR+'/accept',{});"
        "msg.textContent='Решение одобрено.';loadList();}catch(x){err(eEl,x)}}"
        "async function dcApply(){try{if(!CUR)return;await api('POST','/ai-decisions/'+CUR+'/apply',{confirmation:'APPLY_DECISION'});"
        "msg.textContent='Создан черновик процесса (ничего не запущено, live/CRM/бюджет не менялись).';loadList();}catch(x){err(eEl,x)}}"
        "window.dcCreate=dcCreate;window.dcOpen=dcOpen;window.dcAnalyze=dcAnalyze;window.dcSelect=dcSelect;"
        "window.dcReject=dcReject;window.dcAccept=dcAccept;window.dcApply=dcApply;loadList();"
    )
    return _page("AI решения", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/strategy-simulator", response_class=HTMLResponse)
def ui_project_strategy_simulator(project_id: int) -> HTMLResponse:
    """Экран «AI прогноз стратегии» (AI Strategy Simulator v0.7.5)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/decisions'><button class='ghost mini'>AI решения</button></a>"
        f"<a href='/ui/projects/{project_id}/business-forecast'>"
        "<button class='ghost mini'>AI прогноз бизнеса</button></a>"
        f"<a href='/ui/projects/{project_id}/operations'>"
        "<button class='ghost mini'>AI Operations</button></a></div>"
        "<h2>AI прогноз стратегии</h2>"
        "<p class='muted'>AI моделирует последствия сценария решения на 30/60/90 дней, строит "
        "прогноз метрик и сравнивает варианты. Прогноз — модельная оценка, не финансовая "
        "гарантия. Ничего не выполняет и не меняет бизнес/CRM/бюджет/рекламу.</p>"
        "<div class='card'><h3>Новая симуляция из сценария</h3><div class='inline'>"
        "<select id='sm-dec'><option value=''>— решение —</option></select>"
        "<select id='sm-scn'><option value=''>— сценарий —</option></select>"
        "<select id='sm-period'><option value='90_days'>90 дней</option>"
        "<option value='60_days'>60 дней</option><option value='30_days'>30 дней</option></select>"
        "<button class='mini' onclick='smCreate()'>Смоделировать</button></div>"
        "<div class='muted' style='margin-top:6px'>Сценарии берутся из AI-решения "
        "(проанализируйте решение на экране «AI решения»).</div></div>"
        "<div class='card'><h3>Симуляции</h3><div id='sm-list' class='muted'>—</div></div>"
        "<div class='card'><h3>Прогноз</h3><div id='sm-fc' class='muted'>—</div>"
        "<div id='sm-conf' class='muted' style='margin-top:6px'></div></div>"
        "<div class='card'><h3>Сравнение сценариев решения</h3>"
        "<div class='inline'><button class='mini sec' onclick='smCompare()'>Сравнить</button>"
        "<button class='mini' onclick='smRecommend()'>Рекомендация</button></div>"
        "<div id='sm-cmp' class='muted' style='margin-top:6px'>—</div>"
        "<div id='sm-rec' class='muted' style='margin-top:6px'>—</div></div>"
        "<div id='sm-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};let DEC=null;const eEl=document.getElementById('error');"
        "const msg=document.getElementById('sm-msg');"
        "function lst(id,arr){const el=document.getElementById(id);el.classList.remove('muted');"
        "el.innerHTML=(arr&&arr.length)?arr.map(v=>`<div>• ${esc(''+v)}</div>`).join(''):"
        "\"<span class='muted'>—</span>\";}"
        "async function loadDecs(){const d=await api('GET','/projects/'+PID+'/ai-decisions');"
        "const a=d.decisions||[];const el=document.getElementById('sm-dec');"
        "el.innerHTML=\"<option value=''>— решение —</option>\"+a.map(v=>"
        "`<option value='${v.id}'>#${v.id} ${esc(v.title)} (${esc(v.status)})</option>`).join('');}"
        "async function loadScns(did){const el=document.getElementById('sm-scn');"
        "if(!did){el.innerHTML=\"<option value=''>— сценарий —</option>\";return;}"
        "DEC=did;const d=await api('GET','/ai-decisions/'+did);const s=(d.scenarios||[])."
        "filter(x=>x.status!=='rejected');"
        "el.innerHTML=\"<option value=''>— сценарий —</option>\"+s.map(v=>"
        "`<option value='${v.id}'>${esc(v.title)}</option>`).join('');}"
        "async function loadList(){const d=await api('GET','/projects/'+PID+'/simulations');"
        "const a=d.simulations||[];document.getElementById('sm-list').innerHTML=a.length?"
        "a.map(v=>`<div class='prow'>#${v.id} <b>${esc(v.title)}</b> "
        "<span class='badge'>${esc(v.status)}</span> <span class='badge'>score ${v.overall_score}</span> "
        "<span class='badge'>${esc(v.confidence_level)}</span> "
        "<button class='mini sec' onclick=\"smOpen(${v.id})\">Прогноз</button> "
        "<button class='mini' onclick=\"smRun(${v.id})\">Запустить</button></div>`).join(''):"
        "\"<span class='muted'>Симуляций нет — создайте из сценария.</span>\";}"
        "function fcRows(fc){if(!fc||!fc.length)return \"<span class='muted'>Прогноза нет — запустите симуляцию.</span>\";"
        "let h=\"<table class='kw'><tr><th>метрика</th><th>период</th><th>база</th><th>прогноз</th>"
        '<th>Δ%</th><th>увер.</th></tr>";for(const f of fc){h+=`<tr><td>${esc(f.metric)}</td>'
        "<td>${esc(f.period)}</td><td>${f.baseline_value}</td><td>${f.forecast_value}</td>"
        "<td>${f.change_percent}</td><td>${f.confidence_score}</td></tr>`;}return h+'</table>';}"
        "async function smCreate(){try{const sid=document.getElementById('sm-scn').value;"
        "if(!sid){err(eEl,{detail:'Выберите сценарий'});return;}"
        "const period=document.getElementById('sm-period').value;"
        "const c=await api('POST','/projects/'+PID+'/simulations',{scenario_id:+sid,simulation_period:period});"
        "msg.textContent='Симуляция создана — запускаю…';await smRun(c.id);loadList();}catch(x){err(eEl,x)}}"
        "async function smRun(id){try{const r=await api('POST','/simulations/'+id+'/run',{});"
        "document.getElementById('sm-fc').innerHTML=fcRows(r.forecast);"
        "document.getElementById('sm-fc').classList.remove('muted');"
        "document.getElementById('sm-conf').textContent='Уверенность прогноза: '+r.confidence+'/100 · '+(r.note||'');"
        "loadList();}catch(x){err(eEl,x)}}"
        "async function smOpen(id){try{const d=await api('GET','/simulations/'+id);"
        "document.getElementById('sm-fc').innerHTML=fcRows(d.forecast);"
        "document.getElementById('sm-fc').classList.remove('muted');"
        "const e=await api('GET','/simulations/'+id+'/forecast');"
        "document.getElementById('sm-conf').textContent=(e.explanation&&e.explanation.reasons||[]).join(' · ');}catch(x){err(eEl,x)}}"
        "async function smCompare(){try{if(!DEC){err(eEl,{detail:'Выберите решение'});return;}"
        "const r=await api('POST','/decisions/'+DEC+'/compare-scenarios',{});"
        "const sc=(r.comparison_data||{}).scenarios||[];"
        "document.getElementById('sm-cmp').innerHTML=sc.map(v=>`<div>• <b>${esc(v.title)}</b> — "
        "Strategy Score ${v.strategy_score} (эффект ${v.impact}, риск ${v.risk})</div>`).join('')||'—';}catch(x){err(eEl,x)}}"
        "async function smRecommend(){try{if(!DEC){err(eEl,{detail:'Выберите решение'});return;}"
        "const r=await api('GET','/decisions/'+DEC+'/strategy-recommendation');"
        "const w=r.winner?esc(r.winner.title):'—';"
        "document.getElementById('sm-rec').innerHTML=`<b>Рекомендация:</b> ${w} — score ${r.winner_score}, "
        "уверенность ${r.confidence}. ${esc(r.reason||'')} <span class='muted'>${esc(r.note||'')}</span>`;}catch(x){err(eEl,x)}}"
        "document.getElementById('sm-dec').addEventListener('change',e=>loadScns(e.target.value));"
        "window.smCreate=smCreate;window.smRun=smRun;window.smOpen=smOpen;window.smCompare=smCompare;"
        "window.smRecommend=smRecommend;loadDecs();loadList();"
    )
    return _page("AI прогноз стратегии", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/business-forecast", response_class=HTMLResponse)
def ui_project_business_forecast(project_id: int) -> HTMLResponse:
    """Экран «AI прогноз бизнеса» (AI Business Forecasting Engine v0.7.6)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/strategy-simulator'>"
        ...
        f"<a href='/ui/projects/{project_id}/operations'>"
        "<button class='ghost mini'>AI Operations</button></a></div>"
        "<h2>AI прогноз бизнеса</h2>"
        "<p class='muted'>AI прогнозирует развитие бизнеса на 3/6/12 месяцев по текущему состоянию, "
        "Growth Score, продажам и процессам. Прогноз — модельная оценка, не финансовая гарантия. "
        "Ничего не меняет и не выполняет.</p>"
        "<div class='card'><h3>Новый прогноз</h3><div class='inline'>"
        "<select id='bf-hz'><option value='12_months'>12 месяцев</option>"
        "<option value='6_months'>6 месяцев</option><option value='3_months'>3 месяца</option></select>"
        "<button class='mini' onclick='bfCreate()'>Создать и сгенерировать</button></div>"
        "<div id='bf-list' class='muted' style='margin-top:8px'>—</div></div>"
        "<div class='grid'>"
        "<div class='card'><h3>Текущее состояние</h3><div id='bf-baseline' class='kv'>"
        "<div>Revenue</div><div id='bf-b-rev'>—</div>"
        "<div>Growth Score</div><div id='bf-b-growth'>—</div>"
        "<div>Leads / Sales</div><div id='bf-b-leads'>—</div>"
        "<div>Health</div><div id='bf-b-health'>—</div></div></div>"
        "<div class='card'><h3>Прогноз</h3><div id='bf-outlook' class='muted'>—</div>"
        "<div id='bf-meta' class='muted' style='margin-top:6px'></div></div>"
        "</div>"
        "<div class='card'><h3>KPI</h3><div id='bf-kpi' class='muted'>—</div></div>"
        "<div class='card'><h3>Roadmap</h3><div id='bf-roadmap' class='muted'>—</div></div>"
        "<div class='card'><h3>Риски</h3><div id='bf-risks' class='muted'>—</div></div>"
        "<div id='bf-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};const eEl=document.getElementById('error');"
        "const msg=document.getElementById('bf-msg');"
        "function metricRows(ms){if(!ms||!ms.length)return \"<span class='muted'>KPI нет — сгенерируйте прогноз.</span>\";"
        "let h=\"<table class='kw'><tr><th>метрика</th><th>сейчас</th><th>прогноз</th><th>Δ%</th>"
        '<th>увер.</th></tr>";for(const m of ms){h+=`<tr><td>${esc(m.metric)}</td>'
        "<td>${m.baseline_value}</td><td>${m.forecast_value}</td><td>${m.change_percent}</td>"
        "<td>${m.confidence_score}</td></tr>`;}return h+'</table>';}"
        "function outlookRows(fs){if(!fs)return '—';const hz=['3_months','6_months','12_months'];"
        "let h='';for(const k of hz){const b=fs[k];if(!b)continue;"
        "h+=`<div>• <b>${esc(k)}</b>: revenue ${b.revenue!=null?b.revenue+'%':'—'}, `+"
        "`leads ${b.leads!=null?b.leads+'%':'—'}, customers ${b.customers!=null?b.customers+'%':'—'}</div>`;}"
        "return h||'—';}"
        "function rmRows(rm){if(!rm)return \"<span class='muted'>Roadmap нет — сгенерируйте прогноз.</span>\";"
        "let h='';for(const q of (rm.quarters||[])){h+=`<div class='prow'><b>${esc(q.quarter)}</b> `+"
        "`<span class='badge'>${esc(q.focus||'')}</span> ${esc((q.goals||[]).join(' · '))}</div>`;}"
        "if(rm.milestones&&rm.milestones.length)h+=`<div class='muted' style='margin-top:6px'>Вехи: `+"
        "esc(rm.milestones.join(' · '))+'</div>';return h||'—';}"
        "async function loadList(){const d=await api('GET','/projects/'+PID+'/forecasts');"
        "const a=d.forecasts||[];document.getElementById('bf-list').innerHTML=a.length?"
        "a.map(v=>`<div class='prow'>#${v.id} <b>${esc(v.title)}</b> "
        "<span class='badge'>${esc(v.horizon)}</span> <span class='badge'>риск ${esc(v.risk_level)}</span> "
        "<span class='badge'>увер. ${v.confidence_score}</span> "
        "<button class='mini sec' onclick=\"bfOpen(${v.id})\">Открыть</button></div>`).join(''):"
        "\"<span class='muted'>Прогнозов нет — создайте первый.</span>\";}"
        "function renderBaseline(b){b=b||{};document.getElementById('bf-b-rev').textContent=b.revenue!=null?b.revenue:'—';"
        "document.getElementById('bf-b-growth').textContent=b.growth_score!=null?b.growth_score+'/100':'—';"
        "document.getElementById('bf-b-leads').textContent=(b.leads!=null?b.leads:'—')+' / '+(b.customers!=null?b.customers:'—');"
        "document.getElementById('bf-b-health').textContent=b.health_score!=null?b.health_score+'/100':'—';}"
        "async function bfOpen(id){try{const d=await api('GET','/forecasts/'+id);"
        "const f=d.forecast;renderBaseline(f.baseline_state);"
        "document.getElementById('bf-outlook').innerHTML=outlookRows(f.forecast_state);"
        "document.getElementById('bf-outlook').classList.remove('muted');"
        "document.getElementById('bf-meta').textContent='Риск: '+f.risk_level+' · уверенность '+f.confidence_score+'/100';"
        "document.getElementById('bf-kpi').innerHTML=metricRows(d.metrics);"
        "document.getElementById('bf-kpi').classList.remove('muted');"
        "document.getElementById('bf-roadmap').innerHTML=rmRows(d.roadmap);"
        "document.getElementById('bf-roadmap').classList.remove('muted');"
        "const risks=(d.roadmap&&d.roadmap.risks)||[];"
        "document.getElementById('bf-risks').innerHTML=risks.length?risks.map(r=>`<div>• ${esc(''+r)}</div>`).join(''):"
        "\"<span class='muted'>Рисков не выявлено.</span>\";}catch(x){err(eEl,x)}}"
        "async function bfCreate(){try{const hz=document.getElementById('bf-hz').value;"
        "const c=await api('POST','/projects/'+PID+'/forecasts',{horizon:hz});"
        "msg.textContent='Прогноз создан — генерирую…';"
        "await api('POST','/forecasts/'+c.id+'/generate',{});msg.textContent='Готово.';"
        "loadList();bfOpen(c.id);}catch(x){err(eEl,x)}}"
        "window.bfCreate=bfCreate;window.bfOpen=bfOpen;loadList();"
        "(async()=>{try{const o=await api('GET','/projects/'+PID+'/business-outlook');"
        "renderBaseline(o.baseline);if(o.forecast)bfOpen(o.forecast.id);}catch(e){}})();"
    )
    return _page("AI прогноз бизнеса", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/business-planner", response_class=HTMLResponse)
def ui_project_business_planner(project_id: int) -> HTMLResponse:
    """Экран «AI стратегический план» (AI Business Planner v0.7.7)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/business-forecast'>"
        "<button class='ghost mini'>AI прогноз бизнеса</button></a>"
        f"<a href='/ui/projects/{project_id}/execution'>"
        "<button class='ghost mini'>AI исполнение</button></a>"
        f"<a href='/ui/projects/{project_id}/decisions'><button class='ghost mini'>AI решения</button></a></div>"
        "<h2>AI стратегический план</h2>"
        "<p class='muted'>AI превращает бизнес-цель в стратегический план: gap-анализ, квартальные "
        "цели, KPI и roadmap. План — рекомендация; ничего не выполняется без одобрения. Convert "
        "создаёт лишь ЧЕРНОВИК процесса (live/CRM/бюджет не меняются).</p>"
        "<div class='card'><h3>Новая цель</h3><div class='inline'>"
        "<select id='bp-type'>"
        "<option value='revenue'>Выручка</option><option value='growth'>Рост</option>"
        "<option value='sales'>Продажи</option><option value='marketing'>Маркетинг</option>"
        "<option value='efficiency'>Эффективность</option><option value='operational'>Операционное</option></select>"
        "<input id='bp-title' placeholder='цель'/>"
        "<input id='bp-current' type='number' placeholder='сейчас'/>"
        "<input id='bp-target' type='number' placeholder='цель (target)'/>"
        "<button class='mini' onclick='bpCreate()'>Создать + план</button></div>"
        "<div id='bp-list' class='muted' style='margin-top:8px'>—</div></div>"
        "<div class='card'><h3>Цель</h3><div id='bp-gap' class='kv'>"
        "<div>Target</div><div id='bp-g-target'>—</div>"
        "<div>Current</div><div id='bp-g-current'>—</div>"
        "<div>Gap</div><div id='bp-g-gap'>—</div></div>"
        "<div id='bp-strategy' class='muted' style='margin-top:6px'></div></div>"
        "<div class='card'><h3>Кварталы</h3><div id='bp-quarters' class='muted'>—</div></div>"
        "<div class='card'><h3>Milestones</h3><div id='bp-milestones' class='muted'>—</div></div>"
        "<div class='card'><h3>Управление планом</h3><div class='inline'>"
        "<button class='mini sec' onclick='bpApprove()'>Approve Plan</button>"
        "<button class='mini' onclick='bpConvert()'>Create Workflow Draft</button></div>"
        "<div id='bp-plan-status' class='muted' style='margin-top:6px'></div></div>"
        "<div id='bp-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};let PLAN=null;const eEl=document.getElementById('error');"
        "const msg=document.getElementById('bp-msg');"
        "function qCard(o){const kpi=(o.kpi&&o.kpi[0])||{};"
        "const ms=(o.milestones||[]).map(m=>`<div class='muted'>◦ ${esc(m.title)}</div>`).join('');"
        "return `<div class='card'><b>${esc(o.quarter)}: ${esc(o.title)}</b> "
        "<span class='badge'>${esc(o.priority)}</span> <span class='badge'>${esc(o.status)}</span>"
        "<div class='muted'>KPI: ${esc(kpi.metric||'')} → ${kpi.quarter_target!=null?kpi.quarter_target:'—'}</div>"
        "${ms}</div>`;}"
        "async function loadList(){const d=await api('GET','/projects/'+PID+'/goals');"
        "const a=d.goals||[];document.getElementById('bp-list').innerHTML=a.length?"
        "a.map(v=>`<div class='prow'>#${v.id} <b>${esc(v.title)}</b> "
        "<span class='badge'>${esc(v.goal_type)}</span> <span class='badge'>gap ${v.gap}</span> "
        "<button class='mini sec' onclick=\"bpOpen(${v.id})\">Открыть</button></div>`).join(''):"
        "\"<span class='muted'>Целей нет — создайте первую.</span>\";}"
        "async function bpOpen(goalId){try{const d=await api('GET','/goals/'+goalId);"
        "const g=d.gap||{};document.getElementById('bp-g-target').textContent=g.target;"
        "document.getElementById('bp-g-current').textContent=g.current;"
        "document.getElementById('bp-g-gap').textContent=g.gap+' ('+g.gap_percent+'%)';"
        "const plans=d.plans||[];if(!plans.length){document.getElementById('bp-quarters').innerHTML="
        "\"<span class='muted'>Плана нет — нажмите «Создать + план» или сгенерируйте.</span>\";return;}"
        "await openPlan(plans[0].id);}catch(x){err(eEl,x)}}"
        "async function openPlan(planId){PLAN=planId;const d=await api('GET','/plans/'+planId);"
        "const p=d.plan;const st=p.strategy||{};"
        "document.getElementById('bp-strategy').textContent='Стратегия: '+(st.approach||'')+' · уверенность '+p.confidence_score+'/100';"
        "document.getElementById('bp-quarters').innerHTML=(d.objectives||[]).map(qCard).join('');"
        "document.getElementById('bp-quarters').classList.remove('muted');"
        "const mss=[];for(const o of (d.objectives||[])){for(const m of (o.milestones||[]))mss.push(o.quarter+': '+m.title);}"
        "document.getElementById('bp-milestones').innerHTML=mss.length?mss.map(m=>`<div>• ${esc(m)}</div>`).join(''):'—';"
        "document.getElementById('bp-milestones').classList.remove('muted');"
        "document.getElementById('bp-plan-status').textContent='Статус плана: '+p.status;}"
        "async function bpCreate(){try{const t=document.getElementById('bp-title').value.trim();"
        "if(!t){err(eEl,{detail:'Укажите цель'});return;}"
        "const g=await api('POST','/projects/'+PID+'/goals',{goal_type:document.getElementById('bp-type').value,"
        "title:t,current_value:+document.getElementById('bp-current').value||0,"
        "target_value:+document.getElementById('bp-target').value||0});"
        "msg.textContent='Цель создана — генерирую план…';"
        "await api('POST','/goals/'+g.id+'/plan',{});msg.textContent='Готово.';loadList();bpOpen(g.id);}catch(x){err(eEl,x)}}"
        "async function bpApprove(){try{if(!PLAN){err(eEl,{detail:'Откройте план'});return;}"
        "await api('POST','/plans/'+PLAN+'/approve',{});msg.textContent='План одобрен.';openPlan(PLAN);}catch(x){err(eEl,x)}}"
        "async function bpConvert(){try{if(!PLAN){err(eEl,{detail:'Откройте план'});return;}"
        "await api('POST','/plans/'+PLAN+'/convert-workflow',{confirmation:'CONVERT_PLAN'});"
        "msg.textContent='Создан черновик процесса (ничего не запущено, live/CRM/бюджет не менялись).';}catch(x){err(eEl,x)}}"
        "window.bpCreate=bpCreate;window.bpOpen=bpOpen;window.bpApprove=bpApprove;window.bpConvert=bpConvert;loadList();"
    )
    return _page("AI стратегический план", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/execution", response_class=HTMLResponse)
def ui_project_execution(project_id: int) -> HTMLResponse:
    """Экран «AI исполнение» (AI Execution Coordinator v0.7.8)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/business-planner'>"
        ...
        "исполнения: цели, задачи, владельцы, сроки, прогресс, блокеры. Координация — совет; задачи "
        "не выполняются автоматически, бизнес/CRM/бюджет не меняются.</p>"
        "<div class='card'><h3>Новый план исполнения</h3><div class='inline'>"
        "<select id='ex-plan'><option value=''>— утверждённый план —</option></select>"
        "<button class='mini' onclick='exCreate()'>Создать + сгенерировать</button></div>"
        "<div class='muted' style='margin-top:6px'>Берётся ТОЛЬКО одобренный стратегический план "
        "(экран «AI стратегический план» → Approve).</div>"
        "<div id='ex-list' class='muted' style='margin-top:8px'>—</div></div>"
        "<div class='card'><h3>План</h3><div id='ex-plan-head' class='kv'>"
        "<div>Статус</div><div id='ex-h-status'>—</div>"
        "<div>Прогресс</div><div id='ex-h-progress'>—</div></div></div>"
        "<div class='card'><h3>Objectives + Tasks</h3><div id='ex-objectives' class='muted'>—</div></div>"
        "<div class='card'><h3>Blockers</h3><div id='ex-blockers' class='muted'>—</div></div>"
        "<div class='card'><h3>AI Coordination</h3><div id='ex-recs' class='muted'>—</div></div>"
        "<div id='ex-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};let EPLAN=null;const eEl=document.getElementById('error');"
        "const msg=document.getElementById('ex-msg');"
        "function taskCard(t){return `<div class='prow'>• ${esc(t.title)} "
        "<span class='badge'>${esc(t.status)}</span> <span class='badge'>${esc(t.priority)}</span> "
        "<span class='badge'>${t.owner_user_id!=null?('👤 '+t.owner_user_id):'нет владельца'}</span> "
        "<button class='mini sec' onclick=\"exDone(${t.id})\">Готово</button></div>`;}"
        "function objCard(o){const ts=(o.tasks||[]).map(taskCard).join('');"
        "return `<div class='card'><b>${esc(o.title)}</b> <span class='badge'>${esc(o.priority)}</span> "
        "<span class='badge'>${esc(o.status)}</span><div style='margin-top:6px'>${ts}</div></div>`;}"
        "async function loadApproved(){try{const d=await api('GET','/projects/'+PID+'/goals');"
        "const goals=d.goals||[];const opts=[];"
        "for(const g of goals){const gd=await api('GET','/goals/'+g.id);"
        "for(const p of (gd.plans||[])){if(p.status==='approved')opts.push(`<option value='${p.id}'>#${p.id} ${esc(g.title)}</option>`);}}"
        "document.getElementById('ex-plan').innerHTML=\"<option value=''>— утверждённый план —</option>\"+opts.join('');}catch(x){}}"
        "async function loadList(){const d=await api('GET','/projects/'+PID+'/execution-plans');"
        "const a=d.execution_plans||[];document.getElementById('ex-list').innerHTML=a.length?"
        "a.map(v=>`<div class='prow'>#${v.id} <b>${esc(v.title)}</b> <span class='badge'>${esc(v.status)}</span> "
        "<span class='badge'>${v.progress_percent}%</span> "
        "<button class='mini sec' onclick=\"exOpen(${v.id})\">Открыть</button></div>`).join(''):"
        "\"<span class='muted'>Планов исполнения нет — создайте из одобренного плана.</span>\";}"
        "async function exOpen(id){try{EPLAN=id;const d=await api('GET','/execution-plans/'+id);"
        "document.getElementById('ex-h-status').textContent=d.plan.status;"
        "document.getElementById('ex-h-progress').textContent=d.plan.progress_percent+'%';"
        "document.getElementById('ex-objectives').innerHTML=(d.objectives||[]).map(objCard).join('')||'—';"
        "document.getElementById('ex-objectives').classList.remove('muted');"
        "const h=await api('GET','/execution-plans/'+id+'/health');"
        "document.getElementById('ex-blockers').innerHTML=(h.blockers||[]).length?"
        "h.blockers.map(b=>`<div>⚠ ${esc(b.title)} — ${esc(b.detail)}</div>`).join(''):"
        "\"<span class='muted'>Блокеров нет.</span>\";"
        "document.getElementById('ex-recs').innerHTML=(h.recommendations||[]).map(r=>`<div>• ${esc(r)}</div>`).join('');"
        "document.getElementById('ex-recs').classList.remove('muted');}catch(x){err(eEl,x)}}"
        "async function exCreate(){try{const sp=document.getElementById('ex-plan').value;"
        "if(!sp){err(eEl,{detail:'Выберите одобренный план'});return;}"
        "const c=await api('POST','/projects/'+PID+'/execution-plans',{strategic_plan_id:+sp});"
        "msg.textContent='План создан — генерирую…';"
        "await api('POST','/execution-plans/'+c.id+'/generate',{});msg.textContent='Готово.';loadList();exOpen(c.id);}catch(x){err(eEl,x)}}"
        "async function exDone(taskId){try{await api('POST','/execution-tasks/'+taskId+'/status',{status:'completed'});"
        "if(EPLAN)exOpen(EPLAN);}catch(x){err(eEl,x)}}"
...
def ui_project_improvement(project_id: int) -> HTMLResponse:
    ...
    body = (
        ...
        "<button class='ghost mini'>AI эффективность</button></a>"
        f"<a href='/ui/projects/{project_id}/decisions'><button class='ghost mini'>AI решения</button></a>"
        f"<a href='/ui/projects/{project_id}/ai-optimization'>"
        "<button class='ghost mini'>AI оптимизация</button></a></div>"
        "<h2>AI улучшения</h2>"
...
@router.get("/projects/{project_id}/ai-optimization", response_class=HTMLResponse)
def ui_project_ai_optimization(project_id: int) -> HTMLResponse:
    """Экран «AI оптимизация» (AI Autonomous Optimization Engine v0.8.1)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/improvement'>"
        ...
        f"<a href='/ui/projects/{project_id}/performance'>"
        "<button class='ghost mini'>AI эффективность</button></a>"
        f"<a href='/ui/projects/{project_id}/optimization-governance'>"
        "<button class='ghost mini'>Governance</button></a></div>"
        "<h2>AI оптимизация</h2>"
        "<p class='muted'>AI оценивает улучшения (impact × confidence − cost − risk), ранжирует их и "
        "формирует эксперименты-гипотезы для проверки эффекта. Только оценка, приоритизация и "
        "измерение — улучшения не применяются, бизнес и KPI не меняются; эксперименты создаются как "
        "черновик и не запускаются автоматически.</p>"
        "<div class='card'><div class='inline'>"
        "<button class='mini' onclick='opAnalyze()'>Проанализировать backlog</button>"
        "<span id='op-status' class='muted'></span></div></div>"
        "<div class='card'><h3>AI Insights</h3><div id='op-insights' class='muted'>—</div></div>"
        "<div class='card'><h3>Priority ranking</h3><div id='op-ranking' class='muted'>—</div></div>"
        "<div class='card'><h3>Improvement backlog</h3><div id='op-backlog' class='muted'>—</div></div>"
        "<div id='op-msg' class='muted'></div><div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};const eEl=document.getElementById('error');"
        "function resBadge(r){const c=r.validation_result;return `<span class='badge'>${esc(c)}</span> "
        "<span class='muted'>факт ${r.actual_value} vs цель ${r.expected_value} (Δ${r.difference})</span>`;}"
        "function expRow(e,results){return `<div class='prow'>#${e.id} <b>${esc(e.metric)}</b>: "
        "база ${e.baseline_value} → цель ${e.target_value} <span class='badge'>${esc(e.status)}</span> "
        "<span class='muted'>${esc(e.hypothesis||'')}</span>"
        "${e.status!=='completed'?"
        "` <input id='av-${e.id}' size='6' placeholder='факт'> "
        "<button class='mini sec' onclick=\"opValidate(${e.id})\">Validate</button>`:''}"
        "${results.length?' '+resBadge(results[0]):''}</div>`;}"
        "async function loadExps(optId,host){const d=await api('GET','/optimizations/'+optId);"
        "const exps=d.experiments||[];if(!exps.length){host.innerHTML=\"<span class='muted'>Экспериментов нет.</span>\";return;}"
        "const rows=[];for(const e of exps){const ed=await api('GET','/optimization-experiments/'+e.id);"
        "rows.push(expRow(e,ed.results||[]));}host.innerHTML=rows.join('');}"
        "function optCard(o){return `<div class='card'><b>${esc(o.title)}</b> "
        "<span class='badge'>score ${o.optimization_score}</span> <span class='badge'>${esc(o.priority)}</span> "
        "<span class='badge'>${esc(o.status)}</span>"
        "<div class='muted'>impact ${o.impact_score} × conf ${o.confidence_score} − cost ${o.cost_score} − risk ${o.risk_score}</div>"
        "<div class='inline'><button class='mini' onclick=\"opExperiment(${o.id})\">Создать эксперимент</button></div>"
        "<div id='exps-${o.id}' class='muted' style='margin-top:6px'>—</div></div>`;}"
        "async function loadRanking(){const d=await api('GET','/projects/'+PID+'/optimizations');"
        "const a=d.optimizations||[];const host=document.getElementById('op-ranking');host.classList.remove('muted');"
        "host.innerHTML=a.length?a.map(optCard).join(''):\"<span class='muted'>Оптимизаций пока нет — запустите анализ.</span>\";"
        "for(const o of a){loadExps(o.id,document.getElementById('exps-'+o.id));}}"
        "async function loadBacklog(){const d=await api('GET','/projects/'+PID+'/improvements');"
        "const a=d.improvements||[];const host=document.getElementById('op-backlog');host.classList.remove('muted');"
        "host.innerHTML=a.length?a.map(i=>`<div class='prow'>• <b>${esc(i.title)}</b> "
        "<span class='badge'>${esc(i.priority)}</span> <span class='badge'>${esc(i.status)}</span></div>`).join(''):"
        "\"<span class='muted'>Backlog улучшений пуст — сначала запустите AI улучшения.</span>\";}"
        "async function refresh(){await loadRanking();await loadBacklog();}"
        "async function opAnalyze(){try{document.getElementById('op-status').textContent='Оцениваю…';"
        "const r=await api('POST','/projects/'+PID+'/optimization/analyze',{});"
        "document.getElementById('op-status').textContent='Готово.';"
        "const ins=document.getElementById('op-insights');ins.classList.remove('muted');"
        "ins.innerHTML=(r.insights||[]).map(x=>`<div>• ${esc(x)}</div>`).join('');await refresh();}catch(x){err(eEl,x)}}"
        "async function opExperiment(id){try{await api('POST','/optimizations/'+id+'/experiment',{});"
        "loadExps(id,document.getElementById('exps-'+id));}catch(x){err(eEl,x)}}"
        "async function opValidate(eid){try{const el=document.getElementById('av-'+eid);"
        "const v=parseFloat(el&&el.value||'0')||0;"
        "await api('POST','/optimization-experiments/'+eid+'/validate',{actual_value:v});refresh();}catch(x){err(eEl,x)}}"
        "window.opAnalyze=opAnalyze;window.opExperiment=opExperiment;window.opValidate=opValidate;refresh();"
    )
    return _page("AI оптимизация", body, script, active="", active_pid=project_id)


@router.get("/projects/{project_id}/optimization-governance", response_class=HTMLResponse)
def ui_project_optimization_governance(project_id: int) -> HTMLResponse:
    """Экран «Optimization Governance» (AI Optimization Governance Engine v0.8.2)."""
    body = (
        f"<div class='inline'><a href='/ui/projects/{project_id}/dashboard'>"
        "<button class='sec mini'>← К проекту</button></a>"
        f"<a href='/ui/projects/{project_id}/ai-optimization'>"
        ...
        "<p class='muted'>Управление портфелем улучшений: review, approval, назначение владельцев и "
        "отслеживание impact. Только управление — улучшения не применяются, эксперименты не "
        "запускаются, бизнес и KPI не меняются.</p>"
        "<div class='card'><div class='inline'>"
        "<button class='mini' onclick='govAnalyze()'>Обновить портфель</button>"
        "<span id='gov-status' class='muted'></span></div></div>"
        "<div class='card'><h3>Portfolio</h3><div id='gov-portfolio' class='kv'>"
        "<div>Всего</div><div id='pf-total'>—</div>"
        "<div>Approved</div><div id='pf-approved'>—</div>"
        "<div>Active</div><div id='pf-active'>—</div>"
        "<div>Completed</div><div id='pf-completed'>—</div>"
        "<div>Средний impact</div><div id='pf-impact'>—</div></div></div>"
        "<div class='card'><h3>AI Insights</h3><div id='gov-insights' class='muted'>—</div></div>"
        "<div class='card'><h3>Review Queue (pending approvals)</h3>"
        "<div id='gov-queue' class='muted'>—</div></div>"
        "<div class='card'><h3>Owners</h3><div id='gov-owners' class='muted'>—</div></div>"
        "<div class='card'><h3>Impact</h3><div id='gov-impact' class='muted'>—</div></div>"
        "<div class='card'><h3>History (decisions)</h3><div id='gov-history' class='muted'>—</div></div>"
        "<div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};const eEl=document.getElementById('error');"
        "function gv(id,v){const el=document.getElementById(id);el.textContent=v;}"
        "async function loadPortfolio(){const d=await api('GET','/projects/'+PID+'/optimization-portfolio');"
        "const m=d.metrics||{};gv('pf-total',m.total||0);gv('pf-approved',m.approved||0);"
        "gv('pf-active',m.active||0);gv('pf-completed',m.completed||0);gv('pf-impact',m.avg_impact_score||0);"
        "const ins=document.getElementById('gov-insights');ins.classList.remove('muted');"
        "ins.innerHTML=(d.insights||[]).map(x=>`<div>• ${esc(x)}</div>`).join('');}"
        "function gRow(g,detail){const rev=(detail.reviews||[]);const imp=(detail.impacts||[]);"
        "return {g:g,rev:rev,imp:imp,asg:(detail.assignments||[])};}"
        "async function loadAll(){const d=await api('GET','/projects/'+PID+'/optimization-governance');"
        "const gs=d.governances||[];const rows=[];"
        "for(const g of gs){const det=await api('GET','/governance/'+g.id);rows.push(gRow(g,det));}"
        "const q=document.getElementById('gov-queue');q.classList.remove('muted');"
        "const pend=rows.filter(r=>r.g.approval_status==='pending');"
        "q.innerHTML=pend.length?pend.map(r=>`<div class='card'>#${r.g.id} опт.${r.g.optimization_id} "
        "<span class='badge'>${esc(r.g.status)}</span> <span class='badge'>${esc(r.g.priority)}</span>"
        "<div class='inline' style='margin-top:6px'>"
        "<button class='mini sec' onclick=\"govReview(${r.g.id})\">Review</button> "
        "<button class='mini' onclick=\"govApprove(${r.g.id})\">Approve</button> "
        "<button class='mini ghost' onclick=\"govReject(${r.g.id})\">Reject</button> "
        "<input id='own-${r.g.id}' size='5' placeholder='user id'> "
        "<button class='mini ghost' onclick=\"govOwner(${r.g.id})\">Owner</button></div></div>`).join(''):"
        "\"<span class='muted'>Нет ожидающих approvals.</span>\";"
        "const ow=document.getElementById('gov-owners');ow.classList.remove('muted');"
        "const owned=rows.filter(r=>r.g.owner_user_id);"
        "ow.innerHTML=owned.length?owned.map(r=>`<div class='prow'>• governance #${r.g.id} → "
        "владелец user ${r.g.owner_user_id} (${r.asg.length} назнач.)</div>`).join(''):"
        "\"<span class='muted'>Владельцы не назначены.</span>\";"
        "const im=document.getElementById('gov-impact');im.classList.remove('muted');"
        "const imps=rows.filter(r=>r.imp.length);"
        "im.innerHTML=imps.length?imps.map(r=>`<div class='prow'>• governance #${r.g.id}: "
        "<span class='badge'>${esc(r.imp[0].status)}</span> impact ${r.imp[0].impact_score}</div>`).join(''):"
        "\"<span class='muted'>Impact пока не измерен.</span>\";"
        "const hi=document.getElementById('gov-history');hi.classList.remove('muted');"
        "const hist=rows.flatMap(r=>r.rev.map(v=>({gid:r.g.id,v:v})));"
        "hi.innerHTML=hist.length?hist.map(h=>`<div class='prow'>• governance #${h.gid}: "
        "<span class='badge'>${esc(h.v.decision)}</span> ${esc(h.v.comment||'')}</div>`).join(''):"
        "\"<span class='muted'>История решений пуста.</span>\";}"
        "async function refresh(){await loadPortfolio();await loadAll();}"
        "async function govAnalyze(){try{document.getElementById('gov-status').textContent='Обновляю…';"
        "await api('POST','/projects/'+PID+'/optimization-governance',{});"
        "document.getElementById('gov-status').textContent='Готово.';await refresh();}catch(x){err(eEl,x)}}"
        "async function govReview(id){try{await api('POST','/governance/'+id+'/review',{decision:'comment',comment:'reviewed'});refresh();}catch(x){err(eEl,x)}}"
        "async function govApprove(id){try{await api('POST','/governance/'+id+'/approve',{});refresh();}catch(x){err(eEl,x)}}"
        "async function govReject(id){try{await api('POST','/governance/'+id+'/reject',{});refresh();}catch(x){err(eEl,x)}}"
        "async function govOwner(id){try{const el=document.getElementById('own-'+id);"
        "const uid=parseInt(el&&el.value||'0');if(!uid)return;"
        "await api('POST','/governance/'+id+'/owner',{owner_user_id:uid});refresh();}catch(x){err(eEl,x)}}"
        "window.govAnalyze=govAnalyze;window.govReview=govReview;window.govApprove=govApprove;"
        "window.govReject=govReject;window.govOwner=govOwner;refresh();"
    )
    return _page("Optimization Governance", body, script, active="", active_pid=project_id)
...
def ui_ceo_dashboard() -> HTMLResponse:
    ...
    body = (
        ...
        "<p class='muted'>Ежедневная сводка владельца. Advisory: AI только анализирует и советует.</p>"
        "<div id='brief-greeting' style='font-weight:600'>—</div>"
        "<div class='muted'>Health: <span id='brief-health'>—</span>/100</div>"
        "<div style='margin-top:6px'>Главное событие: <span id='brief-event' class='muted'>—</span></div>"
        "<h4>Риски</h4><div id='brief-risks' class='muted'>—</div>"
        "<h4>Возможности</h4><div id='brief-opps' class='muted'>—</div>"
        "<h4>Действия на сегодня</h4><div id='brief-actions' class='muted'>—</div>"
        "<h4>Прогноз</h4><div id='brief-forecast' class='muted'>—</div></div>"
        "<div id='error' class='err'></div>"
    )
    script = (
        "const eEl=document.getElementById('error');let WS=0;"
        ...
        "\"<span class='muted'>Прогноз появится после прогона.</span>\";await loadBrief();}"
        "async function loadBrief(){if(!WS)return;let b;try{b=await api('GET','/pilot/'+WS+'/daily-brief');}catch(x){return;}"
        "const g=document.getElementById('brief-greeting');g.textContent=(b.greeting||'')+' '+(b.company_name||'');"
        "document.getElementById('brief-health').textContent=(b.health_score||0);"
        "const ev=document.getElementById('brief-event');ev.classList.remove('muted');ev.textContent=b.main_event||'—';"
        "lst('brief-risks',b.risks,'Критичных рисков нет.');"
        "lst('brief-opps',b.opportunities,'Возможности не выявлены.');"
        "lst('brief-actions',b.today_actions,'Действий нет.');"
        "const bf=b.forecast||{};const bfc=document.getElementById('brief-forecast');bfc.classList.remove('muted');"
        "bfc.innerHTML=bf.available?`<div>Горизонт: ${esc(bf.horizon||'')}, уверенность ${bf.confidence_score}%, риск ${esc(bf.risk_level||'')}</div>`:"
...
def ui_project_review_workload(project_id: int) -> HTMLResponse:
    ...
    script = (
        ...
        "host.innerHTML=`<table class='kw'><thead><tr><th>Ревьюер</th><th>Задач</th><th>Overdue</th>"
...
def ui_advanced() -> HTMLResponse:
    ...
    body = (
        ...
        "администратором).</p>"
        "<a href='/ui/projects'><button class='mini sec'>К проектам</button></a></div>"
        "<div class='pcard'><h3>Telegram live rollout</h3><p class='meta'>Первый реальный live-канал "
        "автопилота. Реальная отправка выключена по умолчанию (TELEGRAM_LIVE_ROLLOUT_ALLOW_REAL_SEND) "
        "и требует глобального TELEGRAM_LIVE_PUBLISHING_ENABLED + подтверждения.</p>"
        "<a href='/ui/projects'><button class='mini sec'>К проектам</button></a></div>"
        "</div>"
    )
...
def ui_project_live_readiness(project_id: int) -> HTMLResponse:
    ...
    body = (
        ...
        "<div class='card'><h3>Telegram — первый live-канал</h3>"
        "<div class='muted' style='margin-bottom:8px'>Проверьте и включите безопасную реальную "
        "публикацию в Telegram (реальная отправка по умолчанию выключена).</div>"
        f"<a href='/ui/projects/{project_id}/telegram-live-rollout'>"
        "<button class='mini sec'>Telegram live rollout</button></a></div>"
...
@router.get("/projects/{project_id}/telegram-live-rollout", response_class=HTMLResponse)
def ui_project_telegram_live_rollout(project_id: int) -> HTMLResponse:
    """Telegram — первый live-канал автопилота: проверка, preview, тест, run-once (под гейтами)."""
    body = (
        f"{_ap_subnav(project_id, 'telegram-live-rollout')}"
        "<div class='hero'><div class='ap-hero'>Telegram: первый live-канал автопилота</div>"
        "<p class='muted'>Здесь проверяется и включается безопасная реальная публикация в Telegram. "
        "По умолчанию реальная отправка выключена.</p></div>"
        "<div class='card'><div class='inline'>"
        "<span id='tg-status' class='ap-status setup'>Загрузка…</span>"
        "<span id='tg-summary' class='pill'>—</span></div>"
        "<div id='tg-note' class='muted' style='margin-top:6px'></div>"
        "<div class='inline' style='margin-top:10px'>"
        "<button class='ap-big-btn sec' onclick='tgCheck()'>Проверить Telegram live</button></div></div>"
        "<div class='grid'>"
        "<div class='pcard'><h3>Telegram подключение</h3><div id='tg-conn' class='meta'>—</div></div>"
        "<div class='pcard'><h3>Готовность проекта</h3><div id='tg-ready' class='meta'>—</div></div>"
        "<div class='pcard'><h3>Последняя попытка</h3><div id='tg-last' class='meta'>—</div></div>"
        "</div>"
        "<div class='card'><h3>Условия публикации</h3><div id='tg-gates' class='muted'>—</div></div>"
        "<div class='card'><h3>Что мешает</h3><div id='tg-blockers' class='muted'>—</div></div>"
        "<div class='card'><h3>Что сделать дальше</h3><div id='tg-next' class='muted'>—</div>"
        "<div class='inline' style='margin-top:10px'>"
        "<button class='ap-big-btn sec' onclick='tgPreview()'>Предварительный просмотр поста</button>"
        "<button class='ap-big-btn sec' onclick='tgRunDry()'>Тестовый запуск без отправки</button></div>"
        "<div id='tg-preview' class='muted' style='margin-top:8px'></div></div>"
        "<div class='card'><h3>Попробовать live один раз</h3>"
        "<p class='muted'>По умолчанию реальная отправка выключена. Реальная отправка сработает "
        "только если production env-флаги включены администратором.</p>"
        "<div class='inline'><input id='tg-confirm' placeholder='ENABLE_TELEGRAM_LIVE' "
        "style='max-width:260px'></div>"
        "<div class='inline' style='margin-top:8px'>"
        "<button class='ap-big-btn' onclick='tgPublishOnce()'>Попробовать live один раз</button></div>"
        "<div id='tg-publish-status' class='muted' style='margin-top:8px'></div></div>"
        "<div class='card'><h3>История попыток</h3><div id='tg-attempts' class='muted'>—</div></div>"
        f"<div class='card'><a href='/ui/projects/{project_id}/live-readiness'>"
        "<button class='mini sec'>Готовность к автопубликации</button></a> "
        f"<a href='/ui/projects/{project_id}/autopilot'><button class='mini ghost'>К автопилоту</button></a></div>"
        "<div id='error' class='err'></div>"
    )
    script = (
        f"const PID={project_id};const eEl=document.getElementById('error');"
        "const toneMap={enabled:'ready',ready:'ready',blocked:'problem',draft:'setup'};"
        "function yn(v){return v?'да':'нет';}"
        "async function tgLoad(){try{const d=await api('GET','/telegram-live-rollout/projects/'+PID);"
        "const st=document.getElementById('tg-status');st.className='ap-status '+(toneMap[d.status]||'setup');"
        "st.textContent=(d.client_summary&&d.client_summary.headline)||d.status;"
        "document.getElementById('tg-summary').textContent=d.rollout_allow_real_send?'реальная отправка вкл':'реальная отправка выкл';"
        "document.getElementById('tg-note').textContent=d.note||'';"
        "const tp=d.telegram_platform_status||{};document.getElementById('tg-conn').textContent="
        "(tp.connected?'Подключено':'Не подключено')+' · токен: '+yn(tp.token_present)+' · канал: '+yn(tp.channel_present);"
        "const r=d.readiness||{};document.getElementById('tg-ready').textContent="
        "(r.ready?'Готов':'Не готов')+' · можно пробовать live: '+yn(r.can_attempt_live);"
        "const la=d.last_attempt;document.getElementById('tg-last').textContent=la?"
        "(la.status+(la.external_url?(' · '+la.external_url):''))"
        ":'Пока нет попыток';"
        "document.getElementById('tg-gates').innerHTML="
        "`<div class='sched-task'>Условия публикации (глобально): <b>${yn(d.global_live_flag_status)}</b></div>`+"
        "`<div class='sched-task'>Live для проекта: <b>${yn(d.project_live_status)}</b></div>`+"
        "`<div class='sched-task'>Live для Telegram: <b>${yn(d.platform_live_status)}</b></div>`+"
        "`<div class='sched-task'>Full-auto live: <b>${yn(d.full_auto_live_status)}</b></div>`+"
        "`<div class='sched-task'>Реальная отправка (rollout): <b>${yn(d.rollout_allow_real_send)}</b></div>`;"
        "const bl=document.getElementById('tg-blockers');bl.classList.remove('muted');"
        ...
        "const nb=d.next_best_action||{};document.getElementById('tg-next').textContent=nb.label||'';"
        "const at=document.getElementById('tg-attempts');at.classList.remove('muted');"
        "at.innerHTML=(d.recent_attempts&&d.recent_attempts.length)?d.recent_attempts.map(a=>`<div class='sched-task'>`+"
        "`<span class='pill'>${esc(a.status)}</span> ${esc(a.trigger)} · ${esc(a.mode)}${a.external_url?(' · '+esc(a.external_url)):''}</div>`).join(''):'<span class=muted>Пока нет попыток.</span>';"
        "}catch(x){err(eEl,x)}}"
        "async function tgCheck(){tgLoad();}"
        "async function tgPreview(){try{const d=await api('POST','/telegram-live-rollout/projects/'+PID+'/preview',{});"
        "const p=d.payload_preview||{};document.getElementById('tg-preview').textContent="
        "p.available?('Текст: '+p.text_length+' симв. · медиа: '+p.media_count+' · отправилось бы: '+yn(p.would_send)):'Нет поста для предпросмотра.';}catch(x){err(eEl,x)}}"
        "async function tgRunDry(){try{const d=await api('POST','/telegram-live-rollout/projects/'+PID+'/run-dry',{});"
        "document.getElementById('tg-preview').textContent='Тестовый запуск: '+d.status+' (реальной отправки нет).';tgLoad();}catch(x){err(eEl,x)}}"
        "async function tgPublishOnce(){try{const d=await api('POST','/telegram-live-rollout/projects/'+PID+'/publish-once-if-allowed',{confirmation:gv('tg-confirm')});"
        "document.getElementById('tg-publish-status').textContent=d.note||('Статус: '+d.status);tgLoad();}catch(x){err(eEl,x)}}"
        "window.tgCheck=tgCheck;window.tgPreview=tgPreview;window.tgRunDry=tgRunDry;window.tgPublishOnce=tgPublishOnce;tgLoad();"
    )
...
def _ap_subnav(project_id: int, current: str) -> str:
    ...
    tg_cls = "sec" if current == "telegram-live-rollout" else "ghost"
    links += (
        f"<a href='/ui/projects/{project_id}/telegram-live-rollout'>"
...
def ui_project_autopilot(project_id: int) -> HTMLResponse:
    ...
    body = (
        ...
        "<div id='ap-tg-live' class='muted' style='margin-top:4px'>Telegram live: загрузка…</div>"
        f"<a href='/ui/projects/{project_id}/live-readiness'><button class='mini sec'>Готовность к автопубликации</button></a> "
        f"<a href='/ui/projects/{project_id}/telegram-live-rollout'><button class='mini sec'>Telegram live</button></a></div>"
    ...
    script = (
        ...
        "try{const tg=await api('GET','/telegram-live-rollout/projects/'+PID+'/effective-status');"
        "const tgEl=document.getElementById('ap-tg-live');tgEl.classList.remove('muted');"
        "tgEl.textContent='Telegram live: '+(tg.can_send_real?'готов':(tg.can_attempt_live?'готов, отправка выключена':'не готов'));}catch(e){}"
