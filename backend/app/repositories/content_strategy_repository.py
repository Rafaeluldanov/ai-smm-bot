"""Репозиторий автономного контент-стратега (v0.6.6): профиль + рекомендации.

Публичные представления без секретов/токенов. Tenant isolation — на сервис/API-слое.
Рекомендации не удаляются; статусы меняются только через сервис (Review → Apply).
"""

from __future__ import annotations

from datetime import UTC, datetime
from typing import Any

from sqlalchemy import select
from sqlalchemy.exc import IntegrityError
from sqlalchemy.orm import Session

from app.models.content_strategy_profile import ContentStrategyProfile
from app.models.content_strategy_recommendation import ContentStrategyRecommendation

# Поля профиля, которые сервис может обновлять (белый список).
_PROFILE_FIELDS: frozenset[str] = frozenset(
    {
        "status",
        "business_goal",
        "target_audience",
        "brand_positioning",
        "content_pillars",
        "preferred_topics",
        "avoided_topics",
        "preferred_formats",
        "preferred_platforms",
        "posting_strategy",
        "seasonality_rules",
        "last_strategy_update",
    }
)


def _now() -> datetime:
    return datetime.now(UTC)


# ---------------------------------------------------------------------------- #
# Profile                                                                      #
# ---------------------------------------------------------------------------- #


def get_profile(db: Session, project_id: int) -> ContentStrategyProfile | None:
    """Профиль стратегии проекта (или None)."""
    stmt = select(ContentStrategyProfile).where(ContentStrategyProfile.project_id == project_id)
    return db.execute(stmt).scalars().first()


def get_or_create_profile(
    db: Session, project_id: int, account_id: int | None = None
) -> ContentStrategyProfile:
    """Получить или создать профиль (race-safe: при гонке ловим IntegrityError)."""
    existing = get_profile(db, project_id)
    if existing is not None:
        return existing
    profile = ContentStrategyProfile(
        project_id=project_id, account_id=account_id, status="learning"
    )
    db.add(profile)
    try:
        db.commit()
    except IntegrityError:  # параллельное создание — берём чужой профиль
        db.rollback()
        existing = get_profile(db, project_id)
        if existing is not None:
            return existing
        raise
    db.refresh(profile)
    return profile


def get_active_strategy(db: Session, project_id: int) -> ContentStrategyProfile | None:
    """Активная стратегия проекта (status=active), если есть."""
    stmt = select(ContentStrategyProfile).where(
        ContentStrategyProfile.project_id == project_id,
        ContentStrategyProfile.status == "active",
    )
    return db.execute(stmt).scalars().first()


def update_profile(
    db: Session, profile: ContentStrategyProfile, **fields: Any
) -> ContentStrategyProfile:
    """Обновить поля профиля (только белый список)."""
    for key, value in fields.items():
        if key in _PROFILE_FIELDS:
            setattr(profile, key, value)
    db.commit()
    db.refresh(profile)
    return profile


# ---------------------------------------------------------------------------- #
# Recommendations                                                              #
# ---------------------------------------------------------------------------- #


def create_recommendation(
    db: Session,
    *,
    project_id: int,
    account_id: int | None,
    recommendation_type: str,
    title: str,
    description: str | None = None,
    priority: int = 0,
    confidence_score: float = 0.0,
    reasoning: list[Any] | None = None,
    source_signals: list[Any] | None = None,
    expected_impact: dict[str, Any] | None = None,
    apply_payload: dict[str, Any] | None = None,
) -> ContentStrategyRecommendation:
    """Создать рекомендацию (status=generated)."""
    rec = ContentStrategyRecommendation(
        project_id=project_id,
        account_id=account_id,
        recommendation_type=recommendation_type,
        status="generated",
        priority=int(priority),
        title=title[:255],
        description=description,
        confidence_score=float(confidence_score or 0.0),
        reasoning=reasoning or [],
        source_signals=source_signals or [],
        expected_impact=expected_impact or {},
        apply_payload=apply_payload or {},
    )
    db.add(rec)
    db.commit()
    db.refresh(rec)
    return rec


def get_recommendation_by_id(
    db: Session, recommendation_id: int
) -> ContentStrategyRecommendation | None:
    """Рекомендация по id (или None)."""
    return db.get(ContentStrategyRecommendation, recommendation_id)


def list_recommendations(
    db: Session,
    project_id: int,
    *,
    status: str | None = None,
    recommendation_type: str | None = None,
    limit: int = 200,
) -> list[ContentStrategyRecommendation]:
    """Рекомендации проекта (свежие сверху), с фильтрами."""
    stmt = select(ContentStrategyRecommendation).where(
        ContentStrategyRecommendation.project_id == project_id
    )
    if status is not None:
        stmt = stmt.where(ContentStrategyRecommendation.status == status)
    if recommendation_type is not None:
        stmt = stmt.where(ContentStrategyRecommendation.recommendation_type == recommendation_type)
    stmt = stmt.order_by(
        ContentStrategyRecommendation.priority.desc(),
        ContentStrategyRecommendation.id.desc(),
    ).limit(max(1, min(limit, 1000)))
    return list(db.execute(stmt).scalars().all())


def set_status(
    db: Session,
    rec: ContentStrategyRecommendation,
    status: str,
    *,
    stamp_reviewed: bool = False,
    stamp_applied: bool = False,
) -> ContentStrategyRecommendation:
    """Сменить статус рекомендации с проставлением меток времени."""
    rec.status = status
    if stamp_reviewed:
        rec.reviewed_at = _now()
    if stamp_applied:
        rec.applied_at = _now()
    db.commit()
    db.refresh(rec)
    return rec


def approve(db: Session, rec: ContentStrategyRecommendation) -> ContentStrategyRecommendation:
    """Одобрить рекомендацию (status=accepted)."""
    return set_status(db, rec, "accepted", stamp_reviewed=True)


def reject(db: Session, rec: ContentStrategyRecommendation) -> ContentStrategyRecommendation:
    """Отклонить рекомендацию (status=rejected)."""
    return set_status(db, rec, "rejected", stamp_reviewed=True)


def apply(db: Session, rec: ContentStrategyRecommendation) -> ContentStrategyRecommendation:
    """Пометить рекомендацию применённой (status=applied)."""
    return set_status(db, rec, "applied", stamp_applied=True)


# ---------------------------------------------------------------------------- #
# Public views                                                                 #
# ---------------------------------------------------------------------------- #


def public_profile_view(profile: ContentStrategyProfile) -> dict[str, Any]:
    """Безопасное представление профиля (без секретов)."""
    return {
        "id": profile.id,
        "project_id": profile.project_id,
        "account_id": profile.account_id,
        "status": profile.status,
        "business_goal": profile.business_goal,
        "target_audience": dict(profile.target_audience or {}),
        "brand_positioning": dict(profile.brand_positioning or {}),
        "content_pillars": list(profile.content_pillars or []),
        "preferred_topics": list(profile.preferred_topics or []),
        "avoided_topics": list(profile.avoided_topics or []),
        "preferred_formats": list(profile.preferred_formats or []),
        "preferred_platforms": list(profile.preferred_platforms or []),
        "posting_strategy": dict(profile.posting_strategy or {}),
        "seasonality_rules": dict(profile.seasonality_rules or {}),
        "last_strategy_update": (
            profile.last_strategy_update.isoformat() if profile.last_strategy_update else None
        ),
        "updated_at": profile.updated_at.isoformat() if profile.updated_at else None,
    }


def public_recommendation_view(rec: ContentStrategyRecommendation) -> dict[str, Any]:
    """Безопасное представление рекомендации."""
    return {
        "id": rec.id,
        "project_id": rec.project_id,
        "recommendation_type": rec.recommendation_type,
        "status": rec.status,
        "priority": rec.priority,
        "title": rec.title,
        "description": rec.description,
        "reasoning": list(rec.reasoning or []),
        "confidence_score": round(float(rec.confidence_score or 0.0), 1),
        "source_signals": list(rec.source_signals or []),
        "expected_impact": dict(rec.expected_impact or {}),
        "reviewed_at": rec.reviewed_at.isoformat() if rec.reviewed_at else None,
        "applied_at": rec.applied_at.isoformat() if rec.applied_at else None,
        "created_at": rec.created_at.isoformat() if rec.created_at else None,
    }
